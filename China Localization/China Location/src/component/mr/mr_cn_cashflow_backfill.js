...
function(search, log, record, runtime, format, pymtParser, pymtCacher, helper, commons, collectorDao, journalCollector, arCollector, apCollector) {
    ...
    function getInputData() {
        ...
        log.debug('getInputData', 'filterExp=' + tranSearch.filterExpression);
        return tranSearch;
    }
    ...
    function updateFilterExpression(savedSearch, periodId) {
        ...
        //The target transaction will be filtered from each collector->matches method
        var otherFilters =  [
            [
                "type",
                "noneof",
        ...
        if (!periodId) {
            log.error('mr_cn_cashflow_backfill', 'No period passed to current script, trigger transaction re-collected created within 10/8/2018 and 10/29/2018');
            var dateRange = getDateRangeStringExp();
            log.debug('getInputData', 'dateRange=' + JSON.stringify(dateRange));
            dateFilter = [
                [
                    "datecreated",
                    "within",
            ...
            log.debug('mr_cn_cashflow_backfill', 'Collect transaction posted in periodId ' + periodId);
            dateFilter = [
                [
                    "postingperiod",
                    "abs",
     ...
     * example context as below
     * {
            "type": "mapreduce.MapContext",
            "isRestarted": false,
            "key": "1",
            "value": "{\"recordType\":null,\"id\":\"1\",\"values\":{\"GROUP(type)\":{\"value\":\"Journal\",\"text\":\"Journal\"},\"GROUP(internalid)\":{\"value\":\"5898\",\"text\":\"5898\"}}}"
    }
     * @since 2015.1
     */
    function map(context) {
        log.debug('mr_cn_cashflow_backfill', 'Enter map, context=' + JSON.stringify(context));
        ...
        var filters = [
            [
                [
                    "custrecord_cfs_pymt_tranid.internalid",
                    "is",
                    tranId
                ],
                "OR",
                [
                    "custrecord_cfs_deposit_tranid.internalid",
                    "is",
                    tranId
                ]
            ]
        ...
        //as column is grouped by transaction internal id, each collected transaction should only return one grouped result
        ...
        if (result.length === 1) {
            ...
            log.debug('mr_cn_cashflow_backfill->map', 'Transaction ' + tranId + ' already collected, return immediately');
            return;
        } else {
            //trigger collected
            log.debug('mr_cn_cashflow_backfill->map', 'Transaction ' + tranId + ' not collected, start processing');
            var cashflow_params = prepareCashflowParams(payload);
            log.debug('map', 'cashflow_params=' + JSON.stringify(cashflow_params));
            triggerCashflowCollect(cashflow_params);
        }
    }
    /**
     * @desc: This part is the same with ss_cn_cashflow_collect->execute with minor changes
     * @param params
     */
    function triggerCashflowCollect(paramsObj) {
        log.audit('mr_cn_cashflow_backfill', 'Enter triggerCashflowCollect, params=' + JSON.stringify(paramsObj));
        ...
        log.debug('triggerCashflowCollect params', 'type=' + type + ', trantype=' + trantype + ', tranid=' + tranid);
        ...
        log.debug('before insertCashFlowReconciliation', 'result=' + JSON.stringify(result));
    ...
    function prepareCashflowParams(payload) {
        ...
        log.debug('mr_cn_cashflow_backfill', 'prepareCashflowParams, transaction=' + JSON.stringify(transaction));
        var triggerType = 'create';
        var pymtAppliedInfo = getPymtApplyingOrAppliedInfo(transactionType, transactionId, triggerType);
        log.debug('mr_cn_cashflow_backfill', 'type=' + transactionType + ',id=' + transactionId);
        var cashflow_params = {
            type: triggerType,
        ...
        if (hasExceedMaxLimit && transaction.recordType === record.Type.VENDOR_PAYMENT) {
        ...
        log.audit('mr_cn_cashflow_backfill', 'Exit prepareCashflowParams, cashflow_params=' + JSON.stringify(cashflow_params));
        return cashflow_params;
    }

    function getPymtApplyingOrAppliedInfo(transactionType, transactionId, triggerType) {
        if (transactionType === 'VendPymt') {
            var transRecord = record.load({
                type: record.Type.VENDOR_PAYMENT,
                id: transactionId
            });
            var pymtAppliedInfo = pymtParser.fetchPymtApplyInfo(transRecord, triggerType);
            log.debug('vendor payment', 'pymtAppliedInfo=' + JSON.stringify(pymtAppliedInfo));
            ...
            log.debug('customer payment', 'pymtAppliedInfo=' + JSON.stringify(pymtAppliedInfo));
        }
    ...
    function summarize(summary) {
        log.debug('enter summarize', 'summary=' + JSON.stringify(summary));
        var errorSummary = [];
        ...
        log.error('Summarize Error Detail : ', JSON.stringify(errorSummary));
    }
