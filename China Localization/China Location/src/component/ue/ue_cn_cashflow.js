...
function(task, pymtParser, pymtCacher, collectorDao, record, apCollector, commons, letssubmittask, helper, file, runtime) {
    /**
    ...
    function afterSubmit(scriptContext) {
        ...
        if (triggerType === 'create' || triggerType === 'edit' || triggerType === 'delete' || triggerType === 'paybills' || scriptContext.type === 'copy') {
            var currentRec = scriptContext.newRecord;
            log.debug('ue_cn_cashflow', 'type=' + currentRec.type + ',id=' + currentRec.id);
...
//                'customercharge'
            ...
            // we have to get credits thru dynamic record due to the system limitation.
            if (currentRec.type === record.Type.VENDOR_PAYMENT) {
                var pymtAppliedInfo = pymtParser.fetchPymtApplyInfo(currentRec, triggerType);
            ...
            if (hasExceedMaxLimit && currentRec.type === record.Type.VENDOR_PAYMENT) {
    ...
    function beforeLoad(scriptContext) {
        if (runtime.executionContext === runtime.ContextType.USER_INTERFACE) {
            if (scriptContext.type === 'create' || scriptContext.type === 'edit' || scriptContext.type === 'copy') {
                ...
                if (currentRec.type === record.Type.VENDOR_PAYMENT || currentRec.type === record.Type.DEPOSIT || currentRec.type ===record.Type.CUSTOMER_PAYMENT) {
                    return;
                }
                ...
                initField.defaultValue = "<script type='text/javascript' src='" + fileObj.url + "'></script>";
            }
        }
    }
