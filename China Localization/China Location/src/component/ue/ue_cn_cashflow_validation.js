...
define([
    ...
    var api = {
        ...
        beforeSubmit: function(scriptContext) {
            ...
            //ignore below validation if done by user interface client script
            if (exeContext !== runtime.ContextType.USEREVENT || scriptContext.type !== 'create') {
                return true;
            }

            var transactionForm = scriptContext.newRecord;
            //Subsidiary filter
            //second filter is journalWhiteList, this params only applies for journal validation logic
    ...
    /**
     * Do not use toValidateTransactionLines in app_cn_cashflow_validator
     * Only handle UE in SI environment
     * Because if server side script call RESTlet
     * the full https URL and user credential needs to provide
     * Detail information please refer to Suite Answer 51013
     */
    function toValidateTransactionLines(newRecord) {
        if (!runtime.isOW()) {
            //call RESTLet in UE require full qualified URL (starts with https) and user credential
