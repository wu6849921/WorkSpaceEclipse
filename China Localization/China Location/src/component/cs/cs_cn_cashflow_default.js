...
define([
    ...
    function fieldChanged(context) {
        ...
        //employee center role no need default cfs item, because this role can not customize permission.
        if (userObj.roleCenter === 'EMPLOYEE' || userObj.roleId === 'employee_center') {
            return;
        }
    ...
    function itemChanged(context) {
        ...
        if (matchIgnoreItemType(itemType)) {
            ...
            if (commons.makecertain(accountId)) {
                ...
                if (commons.makesure(accountLookups.custrecord_n112_cseg_cn_cfi)) {
                    currentRecord.setCurrentSublistValue({
                        sublistId: 'item',
                        fieldId: 'custcol_cseg_cn_cfi',
    ...
    function matchIgnoreItemType(itemType) {
        var ignoreItemTypeArr = [
            ...
            'Subtotal',
            'Description',
            'Payment' // Payment Item has incomeaccount
        ];
    ...
    /**
     * Check if current item matches item types that should default from Item CFS directly.
     * If current item has both income account and expense account, then it should default from account directly.
     */
    function matchDefaultFromItem(item) {
        if (commons.makesure(item.incomeaccount) && commons.makesure(item.expenseaccount))
            return false;
        else
            return true;
    }
