...
function(file, cache, runtime, commons) {
    ...
    function load(params) {
        log.debug('atblresource.js: params', params);
        ...
        if (commons.makesure(params.language)) {
            contents = getCache().get({
                ...
                loader: function() {
                    return file.load({
                        path: 'src/res/atbl/atbl_' + name.toLowerCase() + '_' + params.language + '.json'
                    }).getContents();
                },
                ttl: 1 * 60 * 60
            // 1hr
            ...
            contents = getCache().get({
                ...
                loader: function() {
                    return file.load({
                        path: 'src/res/atbl/atbl_' + targetName(name.toLowerCase()) + '.json'
                    }).getContents();
                },
                ttl: 1 * 60 * 60
            // 1hr
        ...
        log.debug('atblresource.js: loading contents', contents);
        ...
        if (commons.makesure(key)) {
            if (commons.makesureall(jsonObject, key)) {
                log.debug('atblresource.js: value of key', jsonObject[key]);
                return jsonObject[key];
            } else {
                log.audit('atblresource.js: value of key', 'Unable to locate key, default value ' + defaultValue + ' returned.');
                return defaultValue;
            }
        }
        return jsonObject;
