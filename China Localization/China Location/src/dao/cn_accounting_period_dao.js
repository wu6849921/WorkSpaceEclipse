...
function(record, search, runtime, commons, helper, format, logger, constant) {
     ...
     * @param {object} [params] - parameters.
     * @return {object} - return the current period, if none return latest period.
     */
    ...
    function filters(params) {
        var source = commons.isPrimitive(params) || !commons.makecertain(params) ? null : params.source;
        logger.debug('cn_accounting_period_dao.js: params', params, source);
        ...
        if (commons.makesure(params.isinactive)) {
            filters.push(helper.filter('isinactive').is(params.isinactive));
        }
        if (commons.makesure(params.enddate)) {
            filters.push({
                name: 'enddate',
                operator: commons.makesure(params.enddate.operator) ? params.enddate.operator : search.Operator.ON,
        ...
        if (commons.makesure(params.startdate)) {
            filters.push({
                name: 'startdate',
                operator: commons.makesure(params.startdate.operator) ? params.startdate.operator : search.Operator.ON,
        ...
        if (runtime.isMultipleCalendars() && commons.makesure(params.fiscalcalendar)) {
            filters.push(helper.filter('fiscalcalendar').is(params.fiscalcalendar));
        }

        logger.debug('cn_accounting_period_dao.js: filters', filters, source);
        return filters;
    }
    /**
    ...
    function columns(params) {
        ...
        if (commons.makesureall(params, 'startdate', 'sort')) {
            columnList.push(helper.column('startdate').sort(params.startdate.sort).create());
        } else {
            columnList.push(helper.column('startdate').sort(search.Sort.ASC).create());
        }
        columnList.push(helper.column('isyear').sort(search.Sort.DESC).create());
        columnList.push(helper.column('isquarter').sort(search.Sort.DESC).create());
    ...
    function fetchPeriodsAsDropDown(params) {
        log.debug('cn_accounting_period_dao.js: fetchPeriodsAsDropDown', params);
        ...
        for ( var i in results) {
            ...
            options[i] = {
                value: results[i].id,
                text: leadingSpaces + results[i].getValue('periodname')
            };
        }
        return options;
    }
    ...
    function getDateRange(params) {
        ...
        if (commons.makesure(periodRec)) {
            var startdate = format.parseDate(periodRec.getText('startdate'));
            var enddate = format.parseDate(periodRec.getText('enddate'));
