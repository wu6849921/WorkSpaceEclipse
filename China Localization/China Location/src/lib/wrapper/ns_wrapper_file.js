...
function(file, search, commons, helper) {

    var UUID = 'f29f97d8-5f0d-b745-fa7a-9dce97de71da'; // An empty file with this string as its name should exist under src directory if you want to use relative path to load a file.
    var URI = "src/" + UUID;
     ...
     *          path: The relative file path to the file in the installation root. For example, {path: 'src/someDirectory/someFile'}.
     *          id:   One of the following:
     *                  Internal ID of the file as a number or a string.
     *                  The relative file path to the file in the file cabinet. For example, 'SuiteScripts/someDirectory/someFile'.
     * @returns file.File
     */
     ...
     * @param {String} pathToFolder - The relative directory path to the folder in the installation root. 
     * For example, 'src/someDirectory', 'test/someDirectory'.
     * @param {Function} filter - function to filter files
     * @param {Function} sorter - function to sort files
    ...
    function listFiles(pathToFolder, filter, sorter) {
        ...
        var filters = [
            helper.filter('name').is(folderNames[folderNames.length - 1])
        ];
        ...
        for (var i = 0; i < folders.length; i++) {
            ...
            if (hasFoundExpectedFolder || getParentPath(loadedFile.path) === folderAbsolutePath) {
        ...
        log.debug('ns_wrapper_file.js: loadedFiles', JSON.stringify(expectedFiles));

        return commons.ensure(sorter) ? expectedFiles.sort(sorter) : expectedFiles;
    }
