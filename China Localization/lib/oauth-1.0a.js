...
OAuth.prototype.authorize = function(request, token) {
	...
	oauth_data.oauth_signature = this.getSignature(request, token.secret,
			oauth_data);
 ...
 *            request data
 * @param {Object}
 *            token_secret key and secret token
 * @param {Object}
 *            oauth_data OAuth data
 * @return {String} Signature
 */
OAuth.prototype.getSignature = function(request, token_secret, oauth_data) {
	return this.hash_function(this.getBaseString(request, oauth_data), this
...
OAuth.prototype.getBodyHash = function(request, token_secret) {
	var body = typeof request.data === 'string' ? request.data : JSON
...
OAuth.prototype.getParameterString = function(request, oauth_data) {
	...
	if (oauth_data.oauth_body_hash) {
		base_string_data = this.sortObject(this.percentEncodeData(this
				.mergeObject(oauth_data, this.deParamUrl(request.url))));
	} else {
		base_string_data = this.sortObject(this.percentEncodeData(this
				.mergeObject(oauth_data, this.mergeObject(request.data, this
						.deParamUrl(request.url)))));
	}
...
OAuth.prototype.toHeader = function(oauth_data) {
	...
	return {
		Authorization : header_value.substr(0, header_value.length
				- this.parameter_seperator.length)
	// cut the last chars
	};
};
...
OAuth.prototype.getNonce = function() {
	...
	for (var i = 0; i < this.nonce_length; i++) {
		result += word_characters[parseInt(Math.random()
				* word_characters.length, 10)];
	}
