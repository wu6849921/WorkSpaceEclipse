...
function(departmentDao, commons) {
     ...
     * @param {object} [field] - department field.
     * @param {number} [defaultDepartmentId] - department id.
     * @param {number} [subsidiaryId] - subsidiary id.
     * @param {bool} [isCreate] - initialize department from suitelet or not.
     */
    function initDepartment(field, defaultDepartmentId, subsidiaryId, isCreate) {
        if (!field) {
            return;
        }
        ...
        for ( var i in rs) {
            ...
            var isSelected = hasDepartmentId ? parseInt(defaultDepartmentId) === parseInt(internalid) : false;
