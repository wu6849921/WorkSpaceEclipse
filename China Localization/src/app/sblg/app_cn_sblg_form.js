...
define(
		[ '../../dao/cn_subsidiary_dao',
				...
				'../helper/serverWidget_helper', '../../lib/commons',
				...
				'../../dao/cn_location_dao', '../../dao/cn_department_dao',
				'../../dao/cn_class_dao' ],
		function(subsidiaryDAO, serverWidget, helper, commons, runtime, format,
				resource, extDao, reportHelper, commonDate, locationDAO,
				departmentDAO, classDAO) {
			...
			function createForm() {
				...
				form.clientScriptModulePath = '../../component/cs/cs_cn_sblg.js';
				return form;
			}
			...
			function addDateFromField(form, params) {
				var defaultDateFrom = commons.makesureall(userData, 'date',
						'from', 'name') ? userData.date.from.name
						: params.firstDay;
				...
				}).setDefaultValue(defaultDateFrom).updateLayoutType(
						serverWidget.FieldLayoutType.MIDROW).updateDisplaySize(
						10, 280).setMandatory(true);
			}

			function addDateToField(form, params) {
				var defaultDateTo = commons.makesureall(userData, 'date', 'to',
						'name') ? userData.date.to.name : params.lastDay;
				helper.form(form).addField({
					id : 'custpage_dateto',
					label : labels().To,
					type : serverWidget.FieldType.DATE,
					container : 'custpage_main_filter_group'
				}).setDefaultValue(defaultDateTo).updateLayoutType(
						serverWidget.FieldLayoutType.MIDROW).updateDisplaySize(
						10, 280).setMandatory(true);
			}
			...
			function addButtons(form) {
				...
				if (commons.makesure(userData)) {
					...
					if (!commons.makesureall(listContent, 'body', 'rows')
							&& !commons.makesureall(userData,
									'rowsExceedThreshold')) {
			...
			function addSublist(form) {
				var sblgReportSublist = form.addSublist({
					id : 'custpage_report_sublist',
					type : serverWidget.SublistType.LIST,
					label : labels().sblg
				});

				sblgReportSublist.addField({
			...
			function insertRows(form) {
				...
				if (commons.makesure(listContent)) {
					for (var i = 0; i < listContent.body.rows.length; i++) {
						...
						setSublistValue(headerSublist, 'custpage_account_name',
								i, reportHelper.makeBold(currentRow.account,
										currentRow.istotal));
						setSublistValue(headerSublist, 'custpage_type', i,
								reportHelper.makeBold(currentRow.type,
										currentRow.istotal));
						setSublistValue(headerSublist,
								'custpage_document_number', i,
								reportHelper.makeBold(reportHelper
										.generateGLImpactLink(currentRow),
										currentRow.istotal));
						setSublistValue(headerSublist, 'custpage_gl_number', i,
								reportHelper.makeBold(currentRow.glNumber,
										currentRow.istotal));
						setSublistValue(headerSublist, 'custpage_date', i,
								reportHelper.makeBold(currentRow.date,
										currentRow.istotal));
						setSublistValue(headerSublist, 'custpage_memo', i,
								reportHelper.makeBold(currentRow.memo,
										currentRow.istotal));
						setSublistValue(headerSublist, 'custpage_debit_amount',
								i, reportHelper.alignRight(reportHelper
										.makeBold(currentRow.debit,
												currentRow.istotal)));
						setSublistValue(headerSublist,
								'custpage_credit_amount', i, reportHelper
										.alignRight(reportHelper.makeBold(
												currentRow.credit,
												currentRow.istotal)));
						setSublistValue(headerSublist,
								'custpage_balance_direction', i, reportHelper
										.makeBold(currentRow.balance.direction,
												currentRow.istotal));
						setSublistValue(headerSublist,
								'custpage_balance_amount', i, reportHelper
										.alignRight(reportHelper.makeBold(
												currentRow.balance.amount,
												currentRow.istotal)));
					}
				}
			}
			 ...
			 * @param {object}
			 *            [form] - China Subledger form.
			 */
			function addLocationField(form) {
				var defaultLocation = commons.makesureall(userData, 'location',
						'id') ? userData.location.id : null;
				helper.form(form).addField({
					id : 'custpage_location',
					label : labels().Location,
					type : serverWidget.FieldType.SELECT,
					container : 'custpage_secondary_filter_group'
				}).addSelectOptions(getLocations(form)).setDefaultValue(
						defaultLocation).updateLayoutType(
						serverWidget.FieldLayoutType.MIDROW).updateDisplaySize(
						10, 200);
			}
			...
			function addDepartmentField(form) {
				var defaultDepartment = commons.makesureall(userData,
				...
				helper.form(form).addField({
					...
					type : serverWidget.FieldType.SELECT,
					container : 'custpage_secondary_filter_group'
				}).addSelectOptions(getDepartments(form)).setDefaultValue(
						defaultDepartment).updateLayoutType(
						serverWidget.FieldLayoutType.MIDROW).updateDisplaySize(
						10, 200);
			}
			...
			function addClassField(form) {
				var defaultClass = commons.makesureall(userData, 'clasz', 'id') ? userData.clasz.id
						: null;
				helper.form(form).addField({
					id : 'custpage_class',
					label : labels().Clasz,
					type : serverWidget.FieldType.SELECT,
					container : 'custpage_secondary_filter_group'
				}).addSelectOptions(getClasses(form)).setDefaultValue(
						defaultClass).updateLayoutType(
						serverWidget.FieldLayoutType.ENDROW).updateDisplaySize(
						10, 200);
			}
			...
			function addSubsidiaryField(form) {
				if (runtime.isOW()) {
					...
					subsidiaryField
							.addSelectOptions(subsidiaryDAO
									.fetchSubsidiariesWithConsolidatedOfCurrentRoleAsDropdown(labels().Consolidated));
					subsidiaryField.setDefaultValue(getSubsidiaryId(form))
							.updateLayoutType(
			...
			function getSubsidiaryId(form) {
				if (runtime.isOW()) {
					...
					return commons.makesureall(userData, 'subsidiary', 'id') ? userData.subsidiary.id
							: defaultSubsidiary;
				}
				return null;
			}
			...
			function addAccountFromField(form) {
				var defaultAccountFrom = commons.makesureall(userData,
						'account', 'from', 'id') ? userData.account.from.id
						: null;
				helper.form(form).addField({
					...
					type : serverWidget.FieldType.SELECT,
					container : 'custpage_main_filter_group'
				}).addSelectOptions(getAccounts(form)).setDefaultValue(
						defaultAccountFrom).updateLayoutType(
						serverWidget.FieldLayoutType.MIDROW).updateDisplaySize(
						10, 260);
			}

			function addAccountToField(form) {
				var defaultAccountTo = commons.makesureall(userData, 'account',
						'to', 'id') ? userData.account.to.id : null;
				helper.form(form).addField({
					...
					container : 'custpage_main_filter_group'
						...
						defaultAccountTo).updateLayoutType(
						serverWidget.FieldLayoutType.ENDROW).updateDisplaySize(
						10, 260);
			}
			...
			function addHiddenReportDataIdField(form) {
				...
				reportDataIdField
						.setDefaultValue(
								commons.makesure(reportDataId) ? reportDataId
										: '')
