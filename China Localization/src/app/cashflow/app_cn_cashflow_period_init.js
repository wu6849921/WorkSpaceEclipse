...
function(subsidiaryDao, balance_sheet_periodDao, runtime, commons, format, periodDao) {
     ...
     */
    function initPeriod(field, defaultPeriodId, rs, subsidiaryId, isCreate) {
        if (!isCreate) {
        ...
        for ( var i in rs) {
            ...
            var isSelected = commons.makesure(defaultPeriodId) ? id === defaultPeriodId : isCurrentMonth(startDate, endDate);
            if (isSelected) {
    ...
    function fetchPeriodByCalendarId(calendarId, calendarEnable) {
        if (calendarEnable) {
            var periods = balance_sheet_periodDao.fetchAllPeriodsforLOV(calendarId);
        } else {
