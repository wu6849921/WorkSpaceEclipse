...
define([
    ...
    '../../app/cashflow/app_cn_cashflow_cs_adapter',
    'N/ui/message',
...
function(currentRecord, https, dialog, cs_adapter, message) {
    ...
    function refreshCashFlowFilterForm() {
        ...
        if (cs_adapter.isPeriodRangeCrossFiscalYear(form, dialog.alert, prePeriodFromForFiscalCheck, prePeriodToForFiscalCheck))
            return;
        prePeriodFromForFiscalCheck = form.getValue('custpage_periodfrom');
        prePeriodToForFiscalCheck = form.getValue('custpage_periodto');
        cs_adapter.refreshCashFlowFilterForm(form, dialog.alert, https.get.promise, message);
    }
    /**
    ...
    function fieldChanged(context) {
        cs_adapter.fieldChanged(context.currentRecord, currentRecord.get(), context.fieldId, dialog.alert, originalPeriodFrom, originalPeriodTo);
        originalPeriodFrom = form.getValue('custpage_periodfrom');
        originalPeriodTo = form.getValue('custpage_periodto');
    }
