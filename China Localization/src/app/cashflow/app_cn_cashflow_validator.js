 ...
 * @NApiVersion 2.0
 * @NModuleScope Public
 * Extract common parts for cs/ue cashflow validation logic
 */
define([
...
], function(runtime, url, https, commons, search, error) {
    function toValidateTransactionLines(currentRec, journalWhiteList) {
        ...
        try {
            ...
            log.debug("cs_cn_cashflow_ap_ar_validation response from sl", response.body);
        } catch (ex) {
            throw error.create({
        ...
        if (response.body === 'true' || journalWhiteList.indexOf(currentRec.type) > 0) {
            return true;
        } else {
            return false;
        }
    ...
    function isSubsidiaryRequireCfi(currentRec) {
        ...
        var needValidate = commons.makesure(subsidiaryLookup.custrecord_cn_cfi_mandatory) && subsidiaryLookup.custrecord_cn_cfi_mandatory;
        return needValidate;
    }
