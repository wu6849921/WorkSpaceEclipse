...
function(file, cache, runtime, commons) {
    ...
    function load(params) {
        log.debug('incomeresource.js: params', params);
        ...
        if (commons.makesure(params.language)) {
            contents = getCache().get({
                ...
                loader: function() {
                    return file.load({
                        path: 'src/res/income/income_' + name.toLowerCase() + '_' + params.language + '.json'
                    }).getContents();
                },
                ttl: 1 * 60 * 60 // 1hr
            ...
            contents = getCache().get({
                ...
                loader: function() {
                    return file.load({
                        path: 'src/res/income/income_' + targetName(name.toLowerCase()) + '.json'
                    }).getContents();
                },
                ttl: 1 * 60 * 60 // 1hr
        ...
        log.debug('incomeresource.js: loading contents', contents);
        ...
        if (commons.makesure(key)) {
            if (commons.makesureall(jsonObject, key)) {
                log.debug('incomeresource.js: value of key', jsonObject[key]);
                return jsonObject[key];
            } else {
                log.audit('incomeresource.js: value of key', 'Unable to locate key, default value ' + defaultValue + ' returned.');
                return defaultValue;
            }
        }
        return jsonObject;
