    ...
    function load(params) {
        ...
        if(commons.makesure(params.language)) {
            contents = getCache().get({
                ...
                loader: function() {
                    return file.load({
                        path: 'src/res/cashflow/cashflow_' + name.toLowerCase() + '_' + params.language + '.json'
                    }).getContents();
                },
                ttl: 1 * 60 * 60 // 1hr
            });
            ...
            contents = getCache().get({
                ...
                loader: function() {
                    return file.load({
                        path: 'src/res/cashflow/cashflow_' + targetName(name.toLowerCase()) + '.json'
                    }).getContents();
                },
                ttl: 1 * 60 * 60 // 1hr
            });
        ...
        if (commons.makesure(key)) {
            if (commons.makesureall(jsonObject, key)) {
                log.debug('cashflowresource.js: value of key', jsonObject[key]);
                return jsonObject[key];
            } else {
                log.audit('cashflowresource.js: value of key', 'Unable to locate key, default value ' + defaultValue + ' returned.');
                return defaultValue;
            }
        }
        return jsonObject;
