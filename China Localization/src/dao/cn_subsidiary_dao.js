...
function(search, runtime, helper, record, commons) {
    ...
    function getFiscalCalendar(subsidiaryId) {
        ...
        if (commons.makesure(subsidiaries)) {
            var fiscalcalendar = subsidiaries[0].getValue('fiscalcalendar');
            log.debug('cn_subsidiary_dao.js: fiscal calendar', fiscalcalendar);
    ...
    function fetchAllChinaSubsidiaries() {
        ...
        var result = [];
        subsidiarySearch.run().each(function(searchResult) {
            var id = searchResult.id;
            var name = searchResult.getValue('namenohierarchy');
            try {
                record.load({
                ...
                log.error('cn_subsidiary_dao.js: fetchAllChinaSubsidiaries');
            }
            return true;
        });
        return result;
    ...
    function fetchSubsidiariesOfCurrentRole() {
        ...
        for ( var subIdx in originresults) {
            try {
                ...
                //means current login user doesn't under this subsidiary
                continue;
            }
        }
        var options = [];
    ...
    /**
     * @desc add &nbsp in front of  the subsidiaries in order to display in hierarchy structure
     * @return option list.
     */
     ...
     * 
     * @param the consolidated label should be append to original subsidary name
     * @return Array of options. 
     * Consolidated option format is like
     * {value: "["-1", "1", "2", "3"]", text: "Parent Company (Consolidated)"} 
     */
    function fetchSubsidiariesWithConsolidatedOfCurrentRoleAsDropdown(consolidatedLabel) {
        return fetchSubsidiariesOfCurrentRole().reduce(function(results, option, i, allOptions) {
            return results.concat(consolidatedOption(allOptions, option, consolidatedLabel)).concat(option);
        }, []);
    }
    ...
    function consolidatedOption(allOptions, option, consolidatedLabel) {
        ...
        log.debug('cn_subsidiary_dao:fetchSubsidiariesWithConsolidatedOfCurrentRoleAsDropdown', 'consolidated option: ' + JSON.stringify(option));
        return option;
    }

    /**
     * generate option value for consolidated subsidiary
     * match among names with hierarchy to find all children
     *
     * @return String of consolidated id array.
     * [0] is consolidated id with negative value, [1] is id
     * [>1] are all children ids
     */
