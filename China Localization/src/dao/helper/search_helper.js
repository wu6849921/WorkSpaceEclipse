...
function(search, runtime, commons) {
     ...
     * | ----------------- | ----------- | ---------- | ------------- |
     * | run & getRange    | NO          | 100        | 47            |
     * | runPaged & fetch  | NO          | 50         | 60            |
     * | run & each        | 4000        | N/A        | N/A           |
     */
    function resultset(params) {
        ...
        if (!commons.makesure(results) || start < 0 || commons.makesure(end) && start > end) {
            return null;
        }

        var startTime = runtime.clock();
        log.audit('start getting results(' + start + '-' + end + ')...', startTime);
        ...
        log.audit('time spent (ms): search and get results', runtime.clock() - startTime);

        return whole;
    }

    function to(params) {
        if (commons.makesure(params.end) && params.from + SEARCH_RANGE_SIZE > params.end) {
            return params.end;
        } else {
