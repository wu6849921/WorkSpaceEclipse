		...
		function(search, record, commons, helper) {
			...
			function delCashedTransaction(tranid) {
				...
				if (tranArray.length > 1) {
					log.audit('cn_cashflow_collector_dao delCashedTransaction',
							'tranArray.length is ' + tranArray.length);
				}
				for ( var i in tranArray) {
					try {
						...
						log.error('cn_cashflow_collector_dao', 'Delete error: '
								+ ex.message);
						return false;
					}
				}
			...
			/**
			 * @desc delete China Cash Flow Reconciliation whose Mass Parent is
			 *       null.
			 * @return true or false according to delete result.
			 */
			...
			function queryLineCollectedAmt(lineKey) {
				var lineSearch = search.create({
					...
					/**
					 * NSCHINA-3050, the payment might be voided using reversing
					 * Journal. VOID amount should not be accounted for
					 * collected amount. Add mainline is true to make sure that
					 * there are no duplicates. IMPORTANT: If pymt transaction
					 * is Journal Entry type, this search will return duplicate
					 * records.
					 */
					search.createFilter({
						name : 'voided',
				...
				lineSearch.run().each(
						function(result) {
							collectedAmt += commons.toNumber(result
									.getValue(search.createColumn({
										name : 'custrecord_cfs_amount',
										summary : search.Summary.SUM
									})));
						});
				log.debug('queryLineCollectedAmt', 'lineKey=' + lineKey
			...
			// result =
			// commons.toNumber(searchedResult[0].getValue('custrecord_cfs_pymt_credit_amount'));
			// }
			// return result;
			// }
			 ...
			 * @param {Array}
			 *            [paramArray] - the parameter array.
			 * @return true or false according to insert result.
			 */
			function insertCashFlowReconciliation(paramArray) {
				log
						.debug('Mass Save', 'params = '
								+ JSON.stringify(paramArray));
				if (!commons.makecertain(paramArray)
						|| !commons.isArray(paramArray)
						|| paramArray.length === 0) {
					log
							.debug(
									'cn_cashflow_collector_dao : insertCashFlowReconciliation',
									'parameter error');
					return false;
				}
				...
				for ( var paramIdx in paramArray) {
					...
					if (commons.makesure(paramArray[paramIdx]).parentid) {
						cashedTransactionid = paramArray[paramIdx].parentid;
							...
							.makesure(paramArray[paramIdx].depositTranid)) {
						cashedTransactionid = paramArray[paramIdx].depositTranid;
					} else if (commons
							.makesure(paramArray[paramIdx].pymtTranid)) {
						cashedTransactionid = paramArray[paramIdx].pymtTranid;
					} else {
						log
								.error(
										'cn_cashflow_collector_dao : insertCashFlowReconciliation',
										'invalid parameter, both deposit transaction id and payment id are empty');
						continue;
					}
					var massOpsRecord;
				...
				for ( var recordIdx in massRecords) {
					...
					log
							.debug('Mass Save',
									'saved one parent with its details.');
				}
				return true;
			}
			/**
			...
			function deleteConditionedCollectedEntries(params) {
				log
						.debug(
								'cn_cashflow_collector_dao.deleteConditionedCollectedEntries: params',
								params);
				var lineUniqueKey = params.lineUniqueKey;

				var asearch = search.create({
					type : 'customrecord_cn_cashflow_record_detail',
					filters : [ helper.filter('custrecord_cfs_line_unique_key')
							.is(lineUniqueKey) ],
					columns : [ search.createColumn({
						name : 'internalid'
					}) ],
				...
				log
						.debug(
								'cn_cashflow_collector_dao.deleteConditionedCollectedEntries: entries',
								entries);
