		...
		function(cashflowRepDao, commons, formatter, cashflowItemDao, resource) {
			...
			function getRepData(filter) {
				...
				if (commons.makesure(result)) {
					...
					resultArray[46].amount = payOtherCashRelatedToBusinessActivitiesIM.text;
					...
					resultArray[52].amount = cashRecoveredFromInvestment.text;
					...
					resultArray[58].amount = cashReceivedFromInvestmentIncome.text;
					...
					resultArray[65].amount = cashPaidForThePurchaseAndConstructionOfFixedAssetsIntangibleAssetsAndOtherLongTermAssets.text;
					...
					resultArray[69].amount = cashInvestment.text;
					...
					resultArray[79].amount = cashReceivedFromInvestments.text;
					...
					resultArray[82].amount = cashReceivedFromBorrowings.text;
					...
					resultArray[89].amount = cashRepaymentsForDebts.text;
					...
					resultArray[93].amount = cashPaymentsForDistributionOfDividends.text;
					...
					// 四、汇率变动对现金及现金等价物的影响
					var effectOfForeignExchangeRateChanges = parseAmountWithUnit(result[74]);
					resultArray.push({
