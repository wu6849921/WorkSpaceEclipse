...
define([ '../../lib/commons' ], function(commons) {
	...
	/**
	 * E.g.
	 * helper.form(form).embedJavaScript('https://url/to/javascript/file.js');
	 * 
	 * @param {String}
	 *            fileUrl The URL of file to embed
	...
	function embedJavaScript(fileUrl) {
		if (commons.makesure(fileUrl)) {
			...
			this.setDefaultValue('<script type=\'text/javascript\' src="'
					+ fileUrl + '\"></script>');
		}
		return this;
	}
	...
	function moveLine(params) {
		...
		for ( var i in params.fieldIds) {
			if (commons.ensure(params.fieldValue)) {
				var fieldValue = params.fieldValue(this.form, params.from + 1,
						params.fieldIds[i]);
			} else {
