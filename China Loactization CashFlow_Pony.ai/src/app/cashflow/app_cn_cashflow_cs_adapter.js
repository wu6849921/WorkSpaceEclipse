...
define(
		[ '../../lib/wrapper/ns_wrapper_url',
				...
				'../../lib/util', '../../dao/cn_savedreports_dao',
		...
		function(url, runtime, commons, util, reportDao, periodDao, accountDao,
				formatter, error, exportExcelUtil, period_init, search, helper,
				location_init, department_init, class_init) {
			var _loadingMessage;
			...
			function showLoadingMessage(message) {
				if (!_loadingMessage) {
					_loadingMessage = message
							.create({
								title : runtime.getCurrentUser().getPreference(
										'LANGUAGE') === 'zh_CN' ? '\u6570\u636e\u52a0\u8f7d\u4e2d...'
										: 'Loading...',
			 ...
			 * @param {alertfn} -
			 *            dialog.alert.
			 * @param {httpsGetPromise} -
			 *            https.get.promise.
			 * @param {message} -
			 *            N/ui/message module.
			 */
			function refreshCashFlowFilterForm(form, alertfn, httpsGetPromise,
					message) {
				disableButtons(form);
			...
			function runStandardReports(form, alertfn, httpsGetPromise) {
				...
				// unrealizedGainAndLossCurrent:
				// balanceForAllObject.unrealizedGainAndLossCurrent,
				// unrealizedGainAndLossPrior:
				// balanceForAllObject.unrealizedGainAndLossPrior
				// }
				...
				try {
					...
					search.create({
						type : search.Type.ACCOUNTING_PERIOD,
						filters : [ 'internalid', 'anyof', periodToId ],
						columns : [ 'enddate' ]
					}).run().each(function(result) {
					...
					var endBalanceAmt = 0;
					if (startDate && subsidiaryId && endDate) {
...
//					alert(endBalanceAmt);
					var params = {
						...
						reset : false,
						// line 38, Ending balance of cash and cash equivalents
						endBalanceCurrent : endBalanceAmt,
						endBalancePrior : 0,
			...
			function prepareRunStandardReportURL(form) {
				...
				// prepareRunStandardReportURL',
				// form);
				try {
					var runStandardReportsURL = url
							.resolveScript({
								...
								params : {
									...
									// NSCHINA-2428
									// Add single selection for
									// location/department/class on
									// China
									// Cash Flow Page
				...
				// log.debug('app_cn_cashflow_adapter.js:
				// prepareRunStandardReportURL',
				// runStandardReportsURL);
				// alert(runStandardReportsURL);
			...
			function exportExcel(form) {
				...
				if (window.navigator.msSaveOrOpenBlob) {
					var blobData = exportExcelUtil
							.getExportBlobForCashFlow('cashflow_report');
					window.navigator.msSaveOrOpenBlob(blobData, fileName);
				} else {
					var excelUrl = exportExcelUtil
							.getExportCashFlowUrl('cashflow_report');
			...
			function exportCashFlowAsPDF(form, alertfn, postfn) {
				...
				try {
					...
					// });
					var suiteletUrlForPDF = url.resolveScript({
						scriptId : 'customscript_tn_sl_cn_cashflow',
						deploymentId : 'customdeploy_tn_sl_cn_cashflow',
						returnExternalUrl : false,
						params : params
					});
					var httpsResp = postfn({
						url : suiteletUrlForPDF,
						body : document.getElementById('cashflow_header').outerHTML
								+ document.getElementById('cashflow_data').outerHTML
					});
					var blob = exportExcelUtil.b64toBlob(httpsResp.body,
							'application/pdf');
					if (window.navigator.msSaveOrOpenBlob) {
			 ...
			 * @param {alertfn} -
			 *            dialog.alert.
			 * @param {originalPeriodFrom} -
			 *            original period from.
			 * @param {originalPeriodTo} -
			 *            original period to.
			 */
			function fieldChanged(currentRecord, form, fieldId, alertfn,
					originalPeriodFrom, originalPeriodTo) {
				var cacheInfor = form.getValue({
					fieldId : 'custpage_cachefield'
				});
				...
				if (fieldId === 'custpage_subsidiary') {
					if (runtime.isMultipleCalendars()) {
						subsidiaryChangeHandler(currentRecord, cacheInfor,
								alertfn);
					}
					...
					if (commons.makesure(cacheObj.accountPeriods)) {
						...
						from = formatter.parseDate(cacheObj.accountPeriods[form
								.getValue('custpage_periodfrom')].startdate);
						to = formatter.parseDate(cacheObj.accountPeriods[form
								.getValue('custpage_periodto')].enddate);
					} else {
						from = formatter.parseDate(periodDao.getDateRange(form
								.getValue('custpage_periodfrom')).startdate);
						to = formatter.parseDate(periodDao.getDateRange(form
								.getValue('custpage_periodto')).enddate);
					}
			...
			function suiteletUrl(form, params) {
				var format = commons.isPrimitive(params) ? params
						: params.format;
				var reset = commons.isPrimitive(params) ? false : params.reset;
				var endBalanceCurrent = commons.isPrimitive(params) ? null
						: params.endBalanceCurrent;
				var endBalancePrior = commons.isPrimitive(params) ? null
						: params.endBalancePrior;
				var startBalanceCurrent = commons.isPrimitive(params) ? null
						: params.startBalanceCurrent;
				var startBalancePrior = commons.isPrimitive(params) ? null
						: params.startBalancePrior;
				var unrealizedGainAndLossCurrent = commons.isPrimitive(params) ? null
						: params.unrealizedGainAndLossCurrent;
				var unrealizedGainAndLossPrior = commons.isPrimitive(params) ? null
						: params.unrealizedGainAndLossPrior;
				if (reset) {
								...
								params : {
									filter : JSON
											.stringify({
												...
												// NSCHINA-2428
												// Add
												// location/department/classfication
												// filter
												location : form
														.getValue({
															fieldId : 'custpage_location'
			 ...
			 * @param {alertfn} -
			 *            dialog alert.
			 * @param {prePeriodFromForFiscalCheck} -
			 *            previous period from for fiscal check.
			 * @param {prePeriodToForFiscalCheck} -
			 *            previous period to for fiscal check.
			 * @returns return true or false according to check result.
			 */
			function isPeriodRangeCrossFiscalYear(form, alertfn,
					prePeriodFromForFiscalCheck, prePeriodToForFiscalCheck) {

				var cacheInfor = form.getValue({
					fieldId : 'custpage_cachefield'
				});
				var cacheObj = JSON.parse(cacheInfor);
				if (!commons.makesure(form.getValue('custpage_periodfrom'))
						|| !commons
								.makesure(form.getValue('custpage_periodto'))) {
					alertfn({
				...
				if (!isSameFiscalYear) {
					alertfn(
							...
							setPreviousValueToPeriodFromTo(form,
									prePeriodFromForFiscalCheck,
									prePeriodToForFiscalCheck));
			 ...
			 *            previous period to for fiscal check.
			 */
			function setPreviousValueToPeriodFromTo(form,
					prePeriodFromForFiscalCheck, prePeriodToForFiscalCheck) {
				form.setValue({
					fieldId : 'custpage_periodfrom',
			...
			function subsidiaryChangeHandler(currentRecord, cacheInfor, alertfn) {
				...
				var calendarId = period_init.fetchCalendarIdBySubsidiary(
						subsidiary, true);
				...
				// refresh the accounting periods cache because of the
				// subsidiary change
				refreshCacheInfor(currentRecord, rs);
				var defaultPeriodFrom = period_init.initPeriod(periodFieldFrom,
						null, rs, subsidiary, false);
				var defaultPeriodTo = period_init.initPeriod(periodFieldTo,
						null, rs, subsidiary, false);
				...
				// NSCHINA-2428
				// Update select options for location/department/field with
				// subsidiary
				// change
