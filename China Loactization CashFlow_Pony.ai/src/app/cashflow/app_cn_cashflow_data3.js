		...
		function(cashflowRepDao, commons, formatter, cashflowItemDao, resource) {
			...
			function getRepData(filter) {
				...
				if (commons.makesure(result)) {
					...
					totalCashInflowsFromFinancingActivities += result21.value;
					resultArray.push({
					...
					totalCashOutflowsFromFinancingActivities += result26.value;
					resultArray.push({
					...
					// 四、汇率变动对现金及现金等价物的影响
					var effectOfForeignExchangeRateChanges = parseAmountWithUnit(result[31]);
					resultArray.push({
					...
					var difAmount = parseFloat(filter.endBalanceCurrent)
							- (parseFloat(filter.startBalanceCurrent)
							+ parseFloat(addCashAmt.value));
					resultArray.push({
