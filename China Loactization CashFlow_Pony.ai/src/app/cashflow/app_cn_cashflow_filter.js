			...
			var api = {
				isClearCfsItem : function(currentRec, sublistId, fieldId) {
					if (sublistId === 'line') {
						...
						if (getInOrOut(currentCfs) === 'IN'
								|| getInOrOut(currentCfs) === '\u6D41\u5165'
										.valueOf()) {
							if ((fieldId === 'credit' && common
									.toNumber(amount) <= 0)
									|| (fieldId === 'debit' && common
											.toNumber(amount) >= 0)) {
								return true;
							}
						} else if (getInOrOut(currentCfs) === 'OUT'
								|| getInOrOut(currentCfs) === '\u6D41\u51FA'
										.valueOf()) {
							if ((fieldId === 'credit' && common
									.toNumber(amount) >= 0)
									|| (fieldId === 'debit' && common
