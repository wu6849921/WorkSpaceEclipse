...
define(
		...
		function(record, search, commons, helper) {
			...
			/**
			 * @desc fetchAllPeriod where (isAdjust is not true) and if
			 *       calendarId is not null (fiscalcalendar = calendarId)
			 * @param calendarId :
			 *            number if no calendarId , fetchAll
			...
			function columns(params) {
				...
				var columnList = [
						helper.column('startdate').sort(search.Sort.ASC)
								.create(),
						helper.column('enddate').sort(search.Sort.DESC)
								.create(),
						helper.column('isyear').sort(search.Sort.DESC).create(),
						helper.column('isquarter').sort(search.Sort.DESC)
								.create(),
				...
				if (commons.makecertain(params)
						&& commons.makecertain(params.calendarId)) {
					columnList = columnList.concat([ helper.column(
