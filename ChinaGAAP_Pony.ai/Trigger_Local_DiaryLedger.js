...
trigger.local.diaryledger.prototype = {
	...
	writeDiaryLedgerToPDF : function(subsidiaryid, periodid, period, accountid,
			response) {
	...
	GetDiaryLedgerLists : function(filters, year, balance, map) {
		...
		for (var i = 0; i < resultslice.length; i++) {
			...
			if (i == 0) {
				...
				xml += template.GetDiaryLedgerContentHeadXML(year,
						accountnumber + ' ' + account);
				xml += template.GetOpeningBalanceXML(type, com
						.formatCurrency(balance)); // 期初余额
				m++;
			} else {// check the max line numbers between memo and account, took
					// max value and add it to toal lines amount
			...
			balance = parseFloat(balance) + parseFloat(debit)
					- parseFloat(credit);
			// dynamic content
			xml += template.GetDiaryLedgerXML(month, day, number,
					nlapiEscapeXML(memo), account, com.formatCurrency(debit),
					com.formatCurrency(credit), com.formatCurrency(balance));

			// 当日余额
			if ((date != nextdate) || i == resultslice.length - 1) {// last one
																	// record
				xml += template.GetBalanceOfDayXML(com
