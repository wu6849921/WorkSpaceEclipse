...
function main(request, response) {
	...
	var printExcel = request.getParameter('PRINTEXCEL');
	switch (type) {
	case 'VOUCHER':
	...
	case 'SUBLEDGER':
		if (printExcel == 'F') {
			...
			o.writeSubLedgerToPDF(subsidiaryid, periodid, period, accountid,
					response);
		} else if (printExcel == 'T') {
			var o = new trigger.local.subledgerexcel();
			o.writeSubLedgerToExcel(subsidiaryid, periodid, period, accountid,
					response);
		}
		break;
	case 'DIARYLEDGER':
		if (printExcel == 'F') {
			...
			var o = new trigger.local.diaryledgerexcel();
			o.writeDiaryLedgerToExcel(subsidiaryid, periodid, period,
					accountid, response);
		}
		break;
	case 'LEDGER':
