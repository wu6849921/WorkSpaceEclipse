...
function qtyScan(request, response)
{
	if (request.getMethod() == 'GET') 
	{
		...
		html = html + "<script type='text/javascript'>function validateForm(){var scanedQty =document.getElementById('txtqty').value;var qtyValidate = document.getElementById('txtqty').value.split('.'); if(qtyValidate.length > 1){if(parseInt(qtyValidate[1].length) > 5){document.getElementById('div_error').innerHTML ='Quantity can not have more than 5 decimal places.';document.getElementById('txtqty').focus();return false;}} var serialcomponentItemExists =document.getElementById('hdnisSerialComponentitemExist').value;var fooveragecheck = document.getElementById('hdnvFOoverageChecked').value;if(document.getElementById('txtqty').value == ''){document.getElementById('div_error').innerHTML ='Please enter qty';return false;}else if ((document.getElementById('txtqty').value <= 0 || isNaN(document.getElementById('txtqty').value))){document.getElementById('div_error').innerHTML='Please enter valid qty';document.getElementById('txtqty').value = ''; return false;}else if(serialcomponentItemExists=='T' && ((scanedQty) % 1 != 0)){document.getElementById('div_error').innerHTML='Serial component items exists, hence decimal qty is not allowed.';document.getElementById('txtqty').value = ''; return false;}  else if((parseFloat((document.getElementById('txtqty').value)) > parseFloat(document.getElementById('hdnItemRemQty').value)) && fooveragecheck=='F'){ document.getElementById('div_error').innerHTML='Overage picking is not allowed';document.getElementById('txtqty').value = ''; return false; } else{document.getElementById('div_error').innerHTML=''; var tbl = document.getElementById('dynctable'); var lastRow = tbl.rows.length;   var tbl = document.getElementById('dynctable'); document.getElementById('dynctable').style.display = 'block'; if (tbl != null) {for (var i = 1; i < tbl.rows.length; i++) {tbl.rows[i].cells[1].innerHTML =parseFloat(parseFloat(document.getElementById('txtqty').value)*parseFloat(tbl.rows[i].cells[2].innerHTML)).toFixed(5)}}return true; }}function validateNext(){if(document.getElementById('txtqty').value == ''){document.getElementById('div_error').innerHTML ='Please enter qty';return false;}else{return true;}}</script></head><body>"+
