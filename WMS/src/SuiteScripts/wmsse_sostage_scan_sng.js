 ...
 */
function FulfillmentSngStageScan(request, response)
{

	var ctx = nlapiGetContext();
	var sessionobj = ctx.getSessionObject('session');
	...
	if (request.getMethod() == 'GET') 
	{
		...
		if((vBinDetails == null || vBinDetails == 'null' || vBinDetails == '' || vBinDetails == 'undefined' || vBinDetails.length == null 
				|| vBinDetails.length == 0 )&& (errMsg == ''))
		{
		...
		if (sessionobj!=ctx.getUser()) 
		{
			try
			{
				...
				{
					sessionobj=ctx.getUser();
					ctx.setSessionObject('session', sessionobj); 
				}
				// Processing only if the 'Previous' button is not pressed
				if(optedEvent != 'Back (F7)'){

					nlapiLogExecution('ERROR', 'Inside validate stage bin', enterBin);
						...
						&& opentaskSearchResultsPick !='undefined')
					{
						...
						else
						{
							...
							nlapiLogExecution('ERROR','opentaskSearchResultsPick.length',opentaskSearchResultsPick.length);
							var date=DateStamp();
							...
							if(RecordIdPick!=null && RecordIdPick!='')
							{
								...
								pickStageInventoryBinTransfer(vnSoInternalId,vnItemType,SOarray["custparam_whlocation"],vnFetchedItemId,(Number((Big(vnenterQty).mul(vnConversionRate)).toFixed(8))),vnbinInternalId,vStagebinInternalId,vnBatchNum,vnSerialNum,binTransfer,vInventoryStatus,date);
						...
						if(binTransferId != null && binTransferId != 'null' && binTransferId != '' && binTransferId != 'undefined')
						{
							...
							if(opentaskSearchResultsPick != null && opentaskSearchResultsPick != '' && opentaskSearchResultsPick != 'null' )
							{
								for(var x1=0; x1 < opentaskSearchResultsPick.length; x1++)
								{
									...
									updateMoveOpenTask(itemType,SOarray["custparam_whlocation"],itemId,quantity,vnbinInternalId,vStagebinInternalId,'',binTransferId,taskType,null,soInternalId);
								}
							}
						}
			...
			{					
				ctx.setSessionObject('session', null);
			}
		}
		else
		{
...
function pickStageInventoryBinTransfer(vnSoInternalId,itemType,whLocation,itemId,quantity,fromBinId,toBinId,batchno,serialno,binTransfer,vInventoryStatus,date)
{
	nlapiLogExecution('ERROR', 'Into pickStageInventoryBinTransfer',vnSoInternalId);
	nlapiLogExecution('DEBUG', 'vInventoryStatus',vInventoryStatus);
	//var skt=nlapiLoadRecord('bintransfer',1076);



	binTransfer.setFieldValue('location', whLocation);
	binTransfer.setFieldValue('trandate', date);
	...
	if(itemType == "inventoryitem" || itemType == "assemblyitem" || itemType == "lotnumberedinventoryitem" || itemType=="lotnumberedassemblyitem" || itemType=='Inventory Item' || itemType=='Assembly/Bill of Materials') 
	{
	...
	else
	{
		...
		nlapiLogExecution('ERROR', 'vSerialArray',vSerialArray);
