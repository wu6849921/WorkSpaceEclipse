...
function PIckingSerial(request, response)
{
	...
	if (request.getMethod() == 'GET') 
	{
		...
		{
			if(vStageScanRequired == "T" && ((Number(getNumber) + 1)==(Number(Big(request.getParameter('custparam_enterQty')).mul(vConversionRate))) ))
			{
		...
		nlapiLogExecution('ERROR', 'getPickreportid', getPickreportid);
			...
			try
			{
				...
				if(optedEvent != 'Back (F7)'){
					...
					if(getSerialNo!=null && getSerialNo!="" && getSerialNo!='null')
					{
						...
						var columnssertemp = new Array();
						columnssertemp[0] = new nlobjSearchColumn('custrecord_wmsse_ser_ordno');
						var SrchRecordTmpSerial = nlapiSearchRecord('customrecord_wmsse_serialentry', 'customsearch_wmsse_serialdetails_search', filterssertemp,columnssertemp);
						...
						//itemfilters.push(new nlobjSearchFilter('location','binnumber','anyof',['@NONE@',SOarray["custparam_whlocation"]]));
							...
							try{
								...
								{	
									...
									for(var z=0;z<soLineItemDetails.length;z++)
									{
										...
										var vsoitemopentaskQty=getopentaskDetailsforItem(SOarray["custparam_sointernalid"],vsoLine,vsoitemId,SOarray["custparam_whlocation"]);	
								...
								if(parseFloat(vSoreminqty) > 0)
								{
									...
									if(isItLastPick!=null && isItLastPick!='' && isItLastPick=="T")
									{
										var id=  updateOpenTask(soInternalId,FetchedItemId,soLineno,Number(Big(enterQty).toFixed(5)),binInternalId,itemType,
												...
												ordType,department,vclass,getPickreportid,null,invtStatus);
									}
									else
									{
...
//									response.sendRedirect('SUITELET', 'customscript_wmsse_soscan', 'customdeploy_wmsse_soscan', false, SOarray);
					...
					lockfilters.push(new nlobjSearchFilter('custrecord_wmsse_lock_flag',null,'is','T'));
					lockfilters.push(new nlobjSearchFilter('custrecord_wmsse_lockuser',null,'anyof',context.getUser()));
					if(invtStatus != null && invtStatus != '' && invtStatus != 'null' &&
							invtStatus !='undefined' && invtStatus != undefined)
					{
						lockfilters.push(new nlobjSearchFilter('custrecord_wmsse_lock_invtstatus',null,'anyof',invtStatus));	
					}
					var lockresults= nlapiSearchRecord('customrecord_wmsse_lockbin', 'customsearch_wmsse_binlockingrecords_del', lockfilters, null);
