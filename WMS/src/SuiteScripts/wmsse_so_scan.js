...
function PickingSO(request, response)
{
	if (request.getMethod() == 'GET') 
	{
		...
		logmsg = logmsg + 'getPickreportid. = ' + getPickreportid + '<br>';		
		...
		html = html + "<script type='text/javascript'>function validateForm(){if(document.getElementById('enterso').value == ''){" +
				"if(document.getElementById('hdnTrantype').value=='salesorder'){" +
				"document.getElementById('div_error').innerHTML ='Please enter/scan sales order#/pick report#';}else{" +
				"document.getElementById('div_error').innerHTML ='Please enter/scan transfer order#/pick report#';}return false;}" +
				"else{this.form.submit();this.form.cmdPrevious.disabled=true; return true;}}</script>";
		...
		else
		{
			...
			{	
				...
				{
					vPickRptNo=getSOid;
					for(var z=0;z<vSearchResults.length;z++)
					{	
						var vTaskAssignTo=vSearchResults[z].getValue('custrecord_wmsse_task_assignedto');
						var vTaskAssignToText=vSearchResults[z].getText('custrecord_wmsse_task_assignedto');
						 customer=vSearchResults[z].getValue('custrecord_wmsse_customer');
						var vUser=getCurrentUser();
						
						if((vTaskAssignTo == vUser)||(vTaskAssignTo == null || vTaskAssignTo =='' || vTaskAssignTo =='null' || vTaskAssignTo =='- None -'))
						{
							
							getSOid = vSearchResults[z].getValue('custrecord_wmsse_order_no');
					
							if(vOrderArr.indexOf(getSOid)==-1)
								vOrderArr.push(getSOid);
							validSo=vSearchResults;
			...
			if(validSo!=null && validSo!=''){
				...
				if(customer==null || customer=='')
					customer = validSo[0].getValue('entity');


				if(vOrderArr.length == null || vOrderArr.length == ' ' || vOrderArr.length =='null' || vOrderArr.length == '0')
				{
					vOrderArr.push(getSOid);
				...
				var logmsg = 'customer. = ' + customer + '<br>';
				logmsg = logmsg + 'vOrderArr. = ' + vOrderArr + '<br>';
				logmsg = logmsg + 'vOrdFound. = ' + vOrdFound + '<br>';		
				logmsg = logmsg + 'vType. = ' + vType + '<br>';			
				nlapiLogExecution('DEBUG', 'Values :', logmsg);
				for(var j=0;j<vOrderArr.length;j++)
				{
					getSOid=vOrderArr[j];
					var vType='SalesOrd';
					...
					//var SORecinternalids=getSOLineDetailsItem(getSOid,tranType,null,whLocation);	
					var SORecinternalids = getSOscandetails(getSOid,tranType,whLocation);

					if(SORecinternalids!=null && SORecinternalids!='' && SORecinternalids.length>0)
					{
						nlapiLogExecution('DEBUG','SORecinternalids',SORecinternalids.length);
						var vLinecount=0;
						var vColumnslist=SORecinternalids[0].getAllColumns();
						var vOpentaskdetails = getPickedqtydetails(getSOid,tranType,whlocation);
						...
						{						
							...
							if(parseFloat(vsoitemRemQty)>0 && parseFloat(vsoitemRemQty) <= parseFloat(vsoitemQty))
							{
							...
							if(crossSubsidiaryFeature ==true && tranType=='salesorder')
								 whLocation= SORecinternalids[vSOresults].getValue('inventorylocation');
							else
								 whLocation= SORecinternalids[vSOresults].getValue('location');
						...
						if(parseFloat(vLinecount)>0)
						{
							...
							if(PickReportDetails != null && PickReportDetails != '' && PickReportDetails.length > 0)
							{
								...
								if(PickReportDetails != null && PickReportDetails != '' && PickReportDetails.length != null  && PickReportDetails.length != 'null' && (PickReportDetails.length == 1 || isPickReportNoScanned =='T'))
								{


									if(PickReportDetails.length == 1)
										SOarray["custparam_pickreportid"] = PickReportDetails[0].getValue('custrecord_wmsse_pickreport_no',null,'group');										
									else
										SOarray["custparam_pickreportid"] = vPickRptNo;
								...
								else if(PickReportDetails != null && PickReportDetails != '' && PickReportDetails.length != null && PickReportDetails.length != 1)
								{
									SOarray["custparam_error"] ='';
								...
								else
								{
									...
									if(tranType == 'salesorder')
										...
										SOarray["custparam_error"] = "There are no pick reports generated for this transfer order#";	
						...
						{
							if((vOrderArr.length == (j+1)) && vOrdFound =='F')
							{
								if(tranType == 'salesorder')
								{
									SOarray["custparam_error"] = "Please enter valid sales order#";
								}
								else
								{
									SOarray["custparam_error"] = "Please enter valid order#";	
								}
				...
				if(vOrdFound == 'F')
				{
					nlapiLogExecution('DEBUG','Inside if',vOrdFound);
					if(tranType == 'salesorder')
