...
function PickingSOList(request, response)
{
	if (request.getMethod() == 'GET') 
	{
		...
		var SORecinternalids=getSOList(vTranType,whLocation,null,crossSubsidiaryFeature);
		...
		"{document.getElementById('div_error').innerHTML ='Please enter/scan sales order#';}" +
		"else{document.getElementById('div_error').innerHTML ='Please enter/scan transfer order#';}return false;}" +
		"else{document.getElementById('hdnSOID').value=soID;document.getElementById('hdnShipVia').value=shipVia;" +
		...
		"			<tr><td class='labelmsg'><div id='div_error'  style='color:red;display:block;' >"+errMsg+"</div>" +
		...
		if(SORecinternalids!=null && SORecinternalids!='' && SORecinternalids.length != null)
		{
			...
			if(SORecinternalids.length>0)
			{
				...
				var vOpentaskdetails = getPickedqtylistdetails(null,vTranType,whLocation);
		...
		else
		{
			...
			if(request.getParameter('cmdNextList')=="Next")
			{
				...
				nlapiLogExecution('DEBUG','hdnorderIdArr',request.getParameter('hdnorderIdArr'));
			...
			var logmsg = 'getSOid. = ' + getSOid + '<br>';
			logmsg = logmsg + 'whlocation. = ' + whlocation + '<br>';
			logmsg = logmsg + 'SOID. = ' + SOID + '<br>';	
...
function getSOLineCountList(trantype,whLocation,OrdIntId,vCrosssubsidiary)
{
