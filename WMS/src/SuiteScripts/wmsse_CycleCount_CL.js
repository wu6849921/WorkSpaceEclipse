	...
	if(makeWhSite == "T")
	{
		...
		if(binIdArr.length > 0)
		{
		  var binIdArray=nswms_GetValidBinInternalId_CYCC(binIdArr,whLocation);
			...
			if(tempFlag=="T")
			{
				alert('You can not create inventory count for stage/wip bin location for these bins: '+binIdArray);
				return false;
			}
		}
	}
...
function ValidateLine(type)
{
	...
	if(makeWhSite == "T")
	{
		...
		if(binVal!=null && binVal!='')
		{
			...
			if(binIdArray!=null && binIdArray!='' && binIdArray != 'null' && binIdArray != 'undefined' && binIdArray.length > 0)
			{
				alert('You can not create inventory count for stage/wip bin location.');
				return false;
			}
