...
function fnPickReport(request, response){
	if (request.getMethod() == 'GET') {
		var form = nlapiCreateForm('Multi-Order Pick Report');
		createform(form);
	...
	else
	{
		...
		{
			nlapiLogExecution('ERROR', 'arrSodetails.length', arrSodetails.length); 
			Sodetails = setPagingForSublist(form,arrSodetails,vshipcomplete,vReprint);
		}
		if(Sodetails !=null && Sodetails !='' && Sodetails.length>0)
		{
			nlapiLogExecution('ERROR', 'Sodetails.length', Sodetails.length); 
			//form.setScript('customscript_wmsse_multiordpickreport');
			form.setScript('customscript_wmsse_multiordpickreport_cl');
			//form.addButton('custpage_print','Print','Printreport('+vQbOrdNo+')');
			...
			//sublist.addField("custpage_itmfamily", "text", "Item Family"); 
...
//			}
	...
	var PDFURL = nlapiResolveURL('SUITELET', 'customscript_wmsse_multiordpickreportpdf', 'customdeploy_wmsse_multiordpickreportpdf');
	var ruleDet = screenURL('MULTIORDERPICKREPORT');
	...
	//PDFURL = PDFURL + '&custparam_order_no='+ orderNum+'&custparam_ordertype='+vOrderType+'&custparam_customer='+vCustomer+'&custparam_shipdate='+vShipDate+'&custpage_nooflines='+vnoofLines+'&custpage_qty='+vQty+'&custpage_shipcomplete='+vshipcomplete;
	PDFURL = PDFURL + '&custparam_order_no='+ orderNum+'&custparam_ordertype='+orderType+'&custparam_trantype='+vTranType+'&custparam_customer='+vCustomer+'&custparam_shipdate='+vShipDate+'&custparam_taskassignedto='+vEmployee+'&custpage_nooflines='+vnoofLines+'&custpage_qty='+vQty+'&custparam_tranid_lst='+ ordStr+'&custpage_shipcomplete='+vshipcomplete+'&custparam_whlocation='+whLocation+'&custparam_reprint='+vrePrint;
	nlapiLogExecution('ERROR', 'PDF URL',PDFURL);	
	window.open(PDFURL);
			...
			if(ordsearchresult!=null)
			{
				...
				if(orderListArrayFltr.indexOf(ordsearchresult.getValue('tranid',null,'group'))==-1 && backordList.indexOf(ordsearchresult.getValue('tranid',null,'group'))==-1)
				{
					...
					if(vReprint == 'F' && parseFloat(vSoreminqty) > 0)
					{
						...
						nlapiLogExecution('DEBUG','qtyToPickGen in Big inkit',qtyToPickGen);
