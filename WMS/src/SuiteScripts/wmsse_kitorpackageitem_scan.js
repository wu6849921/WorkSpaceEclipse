...
function PickingSKU(request, response)
{
	if (request.getMethod() == 'GET') 
	{
		...
		if(vItemDetails !=null && vItemDetails!='' && vItemDetails.length > 0 && arrSoSequencedetails != null &&
				arrSoSequencedetails != '' && arrSoSequencedetails!='null' && arrSoSequencedetails.length >0)
		{
			...
			var checkInURL_1 = nlapiResolveURL('SUITELET', 'customscript_wmsse_kitorpkg_binscan', 'customdeploy_wmsse_kitorpkg_binscan');
			...
			for(var b1=0;b1<arrSoSequencedetails.length;b1++)
			{
				var item = arrSoSequencedetails[b1].getText('custrecord_wmsse_sku');
				var line = arrSoSequencedetails[b1].getValue('custrecord_wmsse_line_no');
				...
				for(var b=0;b<vItemDetails.length;b++)
				{
					...
					var expQty=parseFloat(getenteredQty)*parseFloat(bomQty);
					var parentItemCommitedQty = result.getValue('quantitycommitted',null,'max');
					parentItemCommitedQty = parseFloat(parentItemCommitedQty)*parseFloat(bomQty);
					var vsoitemid='';
					var vMemItemFlag='F';

					if(item ==vsoitem && line == vSoLine )
					{
						...
						if(vMemItemFlag == 'T')
						{
							...
							//var actEnteredQty = parseFloat(getenteredQty)*parseFloat(bomQty);
							var actEnteredQty = Number(Big(getenteredQty).mul(bomQty));
							var remainingQty =0;

							if(parseFloat(kitPickConfirmedQty) <= parseFloat(actEnteredQty))
							{
								//remainingQty = parseFloat(actEnteredQty)-parseFloat(kitPickConfirmedQty);
							...
							//var vSoreminqty = parseFloat(expQty)-(parseFloat(pickedqty)-parseFloat(pickConfirmedQty));
							...
							if(parseFloat(vSoLine) == parseFloat(getSOLineNo))
							{
								if(parseFloat(remainingQty) > 0)
								{
									...
									'&custparam_itemid=' + vsoitemid + '   &custparam_sointernalid=' + getSOInternalId + 
									'& custparam_rem_qty=' + vSoreminqty + '&custparam_rec_qty=' + vsoitemRcvQty + '&custparam_whlocation=' + whLocation  +
									'&custparam_soid=' + getSONo  + '&custparam_trantype=' + trantype + '&custparam_company=' + whCompany + 
									...
									'&custparam_memitempickedqty=' + pickedqty  + '&custparam_lineno=' + getSOLineNo +'&custparam_pickreportid=' +getPickreportid +
									'&custparam_actualbegintime=' +getActualBeginTime+'&custparam_department='+department+'&custparam_class='+vclass+
									'&custparam_customer='+ customer+'&custparam_itemremainingquantity='+ getItemRemainingQty+
									'&custparam_whlocationname='+ whLocationName;
								}
		...
		// logMsg = logMsg + 'Item = ' + SOarray["custparam_soitem"] + '<br>';
				...
				if(sosearch!=null && sosearch!='')
				{
					...
					if(currItem!=null && currItem!='')
					{
						...
						if(vIntId != null && vIntId != '')
						{
							...
							var soLineDetails =
								getSOLineDetails(SOarray["custparam_soid"], trantype,SOarray["custparam_fetcheditemid"],SOarray["custparam_whlocation"],SOarray["custparam_lineno"]);
							...
							if(soLineDetails !=null && soLineDetails !='' && soLineDetails.length != null)
							{
								...
								for(var j=0; j<soLineDetails.length; j++)
								{
									...
									if(vsoitem==coItem)
									{
										...
										var vsoitemRcvQty = parseFloat(result.getValue(vColumnslist[memitemtotalPickedQtyIndex]))+parseFloat(fulfiledQty);
										...
										//var vSoreminqty = parseFloat(vsoitemQty)- parseFloat(vsoitemRcvQty);
...
function  fnGetComponentItemDetails(getItemInternalId)
{
	...
	var searchresults = nlapiSearchRecord( 'item', 'customsearch_wmsse_itemdetails', filters, columns1 );  
...
{
	nlapiLogExecution('ERROR', 'sonumber,trantype,whLocation,itemID',sonumber+","+trantype+","+whLocation+","+itemID);
	...
	if(whLocation!=null && whLocation!='' && whLocation!='undefined' && whLocation!='')
	{
		...
		else
		{
			...
			filters.push(new nlobjSearchFilter('custrecord_wmsse_make_wh_site', 'location', 'is', 'T'));
		}
	}
...
function  getSOLineDetailsBasedOnSequence(SONo,trantype,Pickreportid,lineid)
{
	var filters = new Array();
	var columns = new Array();
	...
	if(lineid != null && lineid != '' && lineid!='undefined' && lineid!='null')
	{		
		filters.push(new nlobjSearchFilter('custrecord_wmsse_line_no', null, 'equalto', lineid));
	}
	var columns = new Array();

	var SOLineDetails = new nlapiSearchRecord('customrecord_wmsse_trn_opentask', 'customsearch_wmsse_getpickreport_details', filters, columns);
