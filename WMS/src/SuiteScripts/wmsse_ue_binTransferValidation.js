/**
 * Script Description:
 * This user event script used to validate the Bin Transfer process
 */
/***************************************************************************
 Copyright � 2017,2018, Oracle and/or its affiliates. All rights reserved. 
 ***************************************************************************/
function binTransferValidation(type)
{
	...
	nlapiLogExecution('ERROR', 'vwarehouseloc,warehouselocId', vwarehouseloc+','+warehouselocId);	
	var trantype = nlapiGetRecordType();
	nlapiLogExecution('ERROR', 'trantype,type', trantype+','+type);
			...
			else if(type == 'delete')	
			{
				try
				{
					...
					for(var deleteLineItr=1;deleteLineItr<=vLintcount; deleteLineItr++)
					{
								...
								itemType !='downloaditem' && itemType != 'Download Item' && itemType != 'giftcertificateitem')
						{
							for(var invDetailItr =1; invDetailItr<= invDetSublength; invDetailItr++)
							{
								...
								/*	if (itemType == "inventoryitem" || itemType == "assemblyitem")
							{
								getOPenTaskPickQty=getOPenTaskPickBinDetails(vItemId,vOldToBinId,warehouselocId,'','',vOldLotSerialNumber);
								nlapiLogExecution('ERROR', 'getOPenTaskPickQty', getOPenTaskPickQty);
							}
							else
							{
								if (itemType == "lotnumberedinventoryitem" || itemType=="lotnumberedassemblyitem")
								{
									pickBinDetailsLot=getOPenTaskPickBinDetailsLot(vItemId,vOldToBinId,warehouselocId,vUnitType,stockUnitText);
								...
								nlapiLogExecution('ERROR', 'ErrorMessage1', errorMessage);
					...
					if(errItem.length > 0)			
					{
						...
						var cannotDelError = nlapiCreateError('CannotDelete',	'Please Do not Delete the Bintransfer, Some Openpicks are available for  '+errorMessage+'   ', true);
						throw cannotDelError;  
					}
...
function GetDetailsforOldRecord()
{
	...
	try{
		...
		if(type == 'edit')
		{
			...
			{
				var vItemId =  oldRecord.getLineItemValue('inventory', 'item', lineItr);
