				...
				for(var cnt=0;cnt<soLineDetails.length;cnt++)
				{
					...
					if(vsoitemMultiShipping == "F")
					{
						...
						else if((parseFloat(itemRemQty)>0))
						{
							...
							SOarray["custparam_error"] ='';
							SOarray["custparam_memitemqty"] = request.getParameter('custparam_memitemqty');
							SOarray["custparam_memitempickedqty"] = itemPickedQty;
...
//					vOrderType,vUnits,null,vPageCount,makeInvAvailFlagFromSelect,VUnitType,blnItemUnit,getStockConversionRate);
		...
		"			</tr>"+
		"		 </table>";
		// Implemented UOM and Inventorystatus Dropdown
	...
	else 
	{
		...
		if((enterBin == "" || enterBin == null || enterBin =="null") && (request.getParameter('hdnPreferBin') !=null && 
				request.getParameter('hdnPreferBin') !='' && request.getParameter('hdnPreferBin')!='null' && request.getParameter('hdnPreferBin')!='undefined'))
		{
					...
					if(enterQty!=null && enterQty!="" && enterQty!='null' && !isNaN(enterQty) && parseFloat(remQty) >= parseFloat(enterQty))
					{
						try
						{
								...
								if(itemType == "inventoryitem" || itemType=="assemblyitem" || ((itemType == "serializedinventoryitem" || itemType=="serializedassemblyitem")))
								{
									try
									{
										...
										if(inventoryStatusFeature ==true)
										{
											...
											if(objBinStatusDetails != null && objBinStatusDetails != '' && objBinStatusDetails != 'null' && objBinStatusDetails != 'undefined' && objBinStatusDetails != undefined && objBinStatusDetails.length ==1)
											{
												...
												|| objBinStatusDetails.length ==1 ))
										{
											...
											if(inventoryStatus != ''&& inventoryStatus != null && inventoryStatus != 'null')
											{
							...
							else if(itemType == "serializedinventoryitem" || itemType=="serializedassemblyitem")
							{
								...
								if(inventoryStatusFeature ==true)
								{
									...
									if(objBinStatusDetailssrch != null && objBinStatusDetailssrch != '' && objBinStatusDetailssrch != 'null' && 
											objBinStatusDetailssrch != 'undefined' && objBinStatusDetailssrch != undefined && objBinStatusDetailssrch.length > 0)
									{
										...
										if(vStatusDetails.length > 1)
										{
													...
													vStatusDetails.length != null && vStatusDetails.length == 1)
											{
												...
												SOarray["custparam_status"]=statusId;
							...
							else 
							{
								...
								var soLineItemDetails =	getSOLineDetailsItemandqty(SOarray["custparam_sointernalid"], 
										trantype,FetchedItemId,SOarray["custparam_whlocation"],soLineno);
								...
								if(parseFloat(vSoreminqty) > 0)
								{
									...
									if(inventoryStatusFeature ==true)
									{
										if(objBinStatusDetails != null && objBinStatusDetails != '' && objBinStatusDetails != 'null' && objBinStatusDetails != 'undefined' && objBinStatusDetails != undefined && objBinStatusDetails.length > 0)
										{
											...
											nlapiLogExecution('ERROR', 'vStatusDetails', vStatusDetails.length);
											if(vStatusDetails.length > 1)
											{
												response.sendRedirect('SUITELET', 'customscript_wmsse_kitpkg_selectsts_sng', 'customdeploy_wmsse_kitpkg_selectsts_sng', false, SOarray);
												return; 
								...
								nlapiLogExecution('ERROR', 'vMultiShipping : vShipVia:selectedStatus ', vMultiShipping + ":" + vShipVia+":"+selectedStatus);
								var isItLastPick='F';
								if(parseFloat(vSoreminqty) > 0)
								{
