...
function RFBinPutawayFromBIN(request, response){
	if (request.getMethod() == 'GET') 
	{
		...
		var   binPutawayRecords =null;
		var getStageBins = fnGetStageBinDetails(whLocation);
		nlapiLogExecution('ERROR', 'getStageBins', getStageBins);
		if(getStageBins != null && getStageBins != '' && getStageBins !='null' && getStageBins != 'undefined' && getStageBins.length != null)
		{
		...
		else
		{
			...
			//POarray["custparam_error"] = "No inbound stage location's found.";
			POarray["custparam_error"] = "No inbound stage location's found with inventory.";
		...
		"		<table width='100%'>"+
		"			<tr><td class='tableheading'>Bin Putaway - From Bin List</td></tr>"+
			...
			if(binPutawayRecords.length>0)
			{
				...
				for (var s = getPageCount; s < binPutawayRecords.length; s++) {
					...
					if(vmakeInvAvailFlag == "T")
						...
						var BeginLocQty=binPutawayRecords[s].getValue('quantityonhand','binOnHand','sum');
										
					var checkInURL_1 = nlapiResolveURL('SUITELET', 'customscript_wmsse_bp_item_scan', 'customdeploy_wmsse_bp_item_scan');

					var linkURL_1 = checkInURL_1 + '&custparam_whlocation=' + whLocation + '&custparam_whlocationname=' + whLocationName + '&custparam_language=' + getLanguage + '&custparam_enterBin=' + BeginLoc + '&custparam_actualbegintime=' + actualBeginTime;
		...
		if(optedEvent != 'Back (F7)'){
			if (enterBin != null && enterBin!="") {
				...
				//	r.push(new nlobjSeafilterchFilter('custrecord_wmsse_bin_stg_direction','null','anyof',['1']));
...
function getFromBins(stageBins,whLocation)
{
	...
	/*if(whLocation!= null && whLocation!= '')
		filterStrat.push(new nlobjSearchFilter('location','binnumber', 'anyof', whLocation));
	filterStrat.push(new nlobjSearchFilter('binonhandavail',null,'greaterthan',0));
	filterStrat.push(new nlobjSearchFilter('isinactive', null, 'is', 'F'));

	var columnStrat = new Array();
	columnStrat.push(new nlobjSearchColumn('binnumber',null,'group'));
	columnStrat.push(new nlobjSearchColumn('binonhandavail',null,'sum'));
	//columnStrat.push(new nlobjSearchColumn('itemid'));
	var objBinDetails = new nlapiSearchRecord('item','customsearch_wmsse_stageinvlist',filterStrat, columnStrat);*/
