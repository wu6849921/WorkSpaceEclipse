...
function createNewMappingRecords() {
	try {
		...
		for (var i = 1; i <= sublistLenght; i++) {
			...
			//nlapiLogExecution('DEBUG', 'currLineItemSelection', currLineItemSelection);
//			alert('currLineItemSelection'+currLineItemSelection);
			if (currLineItemSelection == 'T') {
				...
				//nlapiLogExecution('DEBUG', 'dptmnt', dptmnt);
//				alert('dptmnt'+dptmnt);
//				lctn = nlapiGetLineItemValue('custpage_sublist_cncoa_mapping',
//						'custpage_lctn', i);
				lctn='';
//				nlapiLogExecution('DEBUG', 'lctn', lctn);
//				alert('lctn'+lctn);
				...
				//nlapiLogExecution('DEBUG', 'needToCreate', needToCreate);
				...
				if (needToCreate) {
					...
					//alert("idOfCreatedRecord"+idOfCreatedRecord);
...
//			alert(isMappingExist);
			...
			if (!isMappingExist) { // if no mappings exist, need to create a
				...
				rslt = true;
					...
					// == 'T'
		// nlapiLogExecution('DEBUG', 'isNeedToCreateNewMapping', 'CHECKING
		// Existing Mapping for isMapped == T for NS_Account ID= ' + nsAccntID +
		// ' and China_Account=' + chinaAccnt + ', mappedDepartment=' + dptmnt +
...
//			alert(isOne2OneMappingExist);
		...
		alert('isNeedToCreateNewMapping'+ex.message);
		return common
...
function isAlreadyExistingMultipleValueMappingWithClassification(chinaAccnt,
	...
	try {
		...
		nlapiLogExecution('DEBUG',
				...
				ex.message);
		alert('isAlreadyExistingMultipleValueMappingWithClassification：'+ex.message);
		common.sendErrorEmail('dev_support@triggerasia.com', 'FinLocalization',
				'TN_CS_CN_COA_MappingForm',
				'isAlreadyExistingMultipleValueMappingWithClassification',
				ex.message);
...
function addMappingToTheWaitingList() {
	try {
		...
		//var lctnName = nlapiGetFieldText('custrecord_trigger_ns_location');// LOCATION
																			// name
...
//				cls);
		var isInputValid = isValidMapping(sbsdry, isO2MMapping, dptmnt,
				cls);
...
//function isValidMapping(sbsdry, isO2MMapping, dptmnt, lctn, cls) {
	function isValidMapping(sbsdry, isO2MMapping, dptmnt, cls) {
	try {
