...
trigger.local.profitloss.prototype = {
	writeProfitToPDF : function(subsidiaryid, subsidiaryname, periodid, period,
		...
		if (periodid && period) {
			...
			// nlapiLogExecution('debug', '本月', 'currentmonthstart -
			// currentmonthend:' + currentmonthstart +'-'+ currentmonthend);
	...
	GetProfitRecords : function(filter, filters) {
		...
		profit.monthinvestmentincome = parseFloat(n6111[1])
				- parseFloat(n6111[0]);
		...
		profit.profitandlossadjust = parseFloat(n6901[1])
				- parseFloat(n6901[0]);
		n6901 = endmap.GetValue('6901').name;
		if (!n6901) {
			n6901 = [ 0, 0 ];
		}
		profit.monthprofitandlossadjust = parseFloat(n6901[1])
				- parseFloat(n6901[0]);
