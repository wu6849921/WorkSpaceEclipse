...
define(
		[ '../../dao/cn_transaction_ap_dao',
				'../../dao/cn_cashflow_collector_dao', '../../lib/commons',
				'../../lib/wrapper/ns_wrapper_format',
				'../../lib/wrapper/ns_wrapper_record',
				'../helper/cashflow_helper' ],
		function(apTransDao, collectorDao, commons, format, record,
				cashflowHelper) {
			// var recordTypeWhiteList = [ record.Type.VENDOR_PAYMENT,
			// record.Type.CHECK, 'VendPymt', 'Check' ];
			...
			function collect(tranid, collectParam, trantype) {
				log.debug('ap_collector', 'Enter collect, collectParam='
						+ JSON.stringify(collectParam));
				if (trantype === record.Type.VENDOR_PAYMENT
						|| trantype === 'VendPymt') {
					...
					var transList = apTransDao.fetchAPTransaction(tranid,
							paidTransIdArray);
					log.debug('ap_collector', 'fetchAPTransaction, transList='
							+ JSON.stringify(transList));
					var result = [];
					for ( var transId in transList) {
						result = result.concat(processBill(transList[transId],
								tranid, collectParam[transId]));
					}
				} else if (trantype === record.Type.CHECK
						|| trantype === 'Check') {
					var transList = apTransDao.fetchAPTransaction(tranid,
							tranid);
					var result = [];
					for ( var transId in transList) {
						result = result.concat(processCheck(transList[transId],
								tranid));
					}
				}
			...
			function processBill(bill, tranid, paymentApplyInfo) {
				...
				mainline.paymentSubsidiary = cashflowHelper
						.getCurrentSubsidiary(paymentApplyInfo.subsidiary);
			...
			function processCheck(check, tranid) {
				...
				mainline.paymentAmount = commons
						.toAbsNumber(mainline.totalAmount);
				mainline.totalAmount = commons
						.toAbsNumber(mainline.totalAmount);
			...
			function saveItemLines(itemLines, mainline) {
				...
				for (var i = 0; i < itemLines.length - 1; i++) {
					...
					saveObj.amount = format.round(item.amount
							* mainline.paymentAmount
							/ (commons.toNumber(mainline.totalAmount) + commons
									.toNumber(mainline.advToApplyAmount)));
					log.debug('saveObj.amount', saveObj.amount);
					paidAmt += saveObj.amount;
					result.push(saveObj);
				}
				// last item line amount = total amount - sum (previous line
				// amount)
			...
			function saveFinalPayItemLines(itemLines, mainline) {
				...
				for (var i = 0; i < itemLines.length; i++) {
					var item = itemLines[i];
					var saveObj = composeLineResult(item, mainline, i);
					var itemcollectedAmt = collectorDao
							.queryLineCollectedAmt(item.lineuniquekey);
				...
				// Right matches there is discount taken in payment or foreign
				// currency rate exchange
				// Both condition should force calculation in percentage
				if (mainline.advToApplyAmount !== 0
						|| ((prevBillCollectedAmt + mainline.paymentAmount) !== mainline.totalAmount)) {
			 ...
			 * 
			 * @param itemLines
			 * @returns {Array} line index for those needs to apply header cfs
			 *          (those lines whose cfs is empty), line index start from
			 *          zero
			 */
