...
define(
		...
		function(templatemanager, render, resource, forms, commons, format,
			...
			function handleAsPage(filter) {
				// log.debug('cash flow handler: handleAsPage: filter', filter);
				// {"subsidiary":"3","subsidiaryName":"Honeycomb Holdings
				// Inc.","unit":"1","unitName":"Unit","location":"","locationName":"","department":"-1","departmentName":"
				// ","classification":"-1","className":"
				// ","period":{"from":"305","to":"305"},"periodName":{"fromName":"Nov
				// 2018","toName":"Nov
				// 2018"},"endBalanceCurrent":0,"endBalancePrior":0,"startBalanceCurrent":0,"startBalancePrior":0,"unrealizedGainAndLossCurrent":null,"unrealizedGainAndLossPrior":0}
				if (commons.makesure(filter)) {// 如果有查询条件
					if (!filter.reset) {
						getRepData(filter);// 得到数据
				...
				return forms.renderAsPage();// 初始化页面
			}
