 ...
 */
define(
		[ '../../dao/cn_cashflow_dao', '../../dao/helper/search_helper',
				'../helper/cashflow_helper',
				'../../lib/wrapper/ns_wrapper_runtime', '../../lib/commons',
				'../../lib/wrapper/ns_wrapper_format',
				'../../lib/wrapper/ns_wrapper_record',
				'../../dao/cn_cashflow_collector_dao' ],
		function(cashflowDAO, helper, cashflowHelper, runtime, commons, format,
				record, collectoDAO) {

			// tranType === record.Type.CUSTOMER_PAYMENT || tranType ===
			// record.Type.DEPOSIT || tranType === record.Type.CUSTOMER_DEPOSIT
			// || tranType === record.Type.CUSTOMER_REFUND || tranType ===
			// record.Type.CASH_SALE || tranType === record.Type.CASH_REFUND ||
			// tranType === 'CustPymt';

			/*
			 * @desc A variable to store reconciliation lines. This will be
			 * auto-cleaned once next transaction is coming. @var {Array}
			 * collectedEntries
			 */
			var collectedEntries = [];

			/*
			 * @desc A variable to store lines of single deposit/refund
			 * transaction like deposit etc. This will be auto-cleaned once next
			 * transaction is coming. @var {Object} depositTranEntries
			 */
			var depositTranEntries = []; // Deposit/Refund Transaction

			/*
			 * @desc A variable to store lines of deposited transaction like
			 * payment, cash sale etc. This will be cleaned when exiting the
			 * file. @var {Object} depositedTranEntries
			 */
			var depositedTranEntries = []; // AppliedTo Transaction

			/*
			 * @desc A variable to store lines of single transaction. This will
			 * be auto-cleaned once next transaction is coming. @var {Array}
			 * tranEntries
			 */
			var tranEntries = [];

			/*
			 * @desc A variable to store lines of single paid transaction like
			 * bill, invoice etc. @var {Object} paidTranEntries
			 */
			var paidTranEntries = {};

			/*
			 * @desc A variable to store lines of single credit transaction like
			 * credit memo etc. @var {Array<search.Result>} creditTranEntries
			 */
			var creditTranEntries = [];

			// Helper Vars //
			/*
			 * @desc A variable to store total paid amount of preceding lines in
			 * an invoice or cash sale. @var {Number} precedingLinePaidAmount
			 */
			var precedingLinePaidAmount = 0;

			/*
			 * @desc A variable to store shipping and gift amount in an invoice
			 * or cash sale. @var {Number} otherExpenses
			 */
			var otherExpenses = 0;

			/*
			 * @desc A variable to store total paid amount of preceding
			 * transactions in an customer payment etc. It is an inner variable
			 * with no any business usage. @var {Number}
			 * precedingTransPaidAmount
			 */
			var precedingTransPaidAmount = 0;

			/*
			 * @desc A variable to store internal id of preceding transaction in
			 * an customer payment etc. It is an inner variable with no any
			 * business usage. @var {Number} preTranEntryId
			 */
			var preTranEntryId = -1;

			/*
			 * @desc A variable to store mapping information from deposited
			 * transaction ID to deposit transaction entry. It is an helper
			 * variable without business concept. @var {Object<String:search.Result>}
			 * depositTranEntryMap
			 */
			...
			function collect(tranid) {
				...
				setup(arguments);
				collectAR();
				log.debug('AR: Totally collected entries',
			...
			function setup(params) {
				...
				tranEntries = helper.resultset(cashflowDAO
						.fetchTransactionEntries({
							ids : params[0],
							hasdeposit : params[2] === record.Type.DEPOSIT
						}));
				log.debug('AR: returned raw Deposit|Paying results',
						tranEntries);
				...
				if (tranEntries[0].getValue('type') === 'Deposit') {
					...
					depositedTranEntries = helper.resultset(cashflowDAO
							.fetchTransactionEntries({
								depositTransaction : tranEntries[0].id
							}));
					log.debug('AR: returned raw Deposited(Paying) results',
				...
				if (tranEntries[0].getValue('type') === 'Deposit') {
					...
					log
							.debug('Other Transactio Type: payingTranEntries initialized');
				...
				if (commons.makesure(paidTranIds)) {
					var paidEntries = helper.resultset(cashflowDAO
							.fetchTransactionEntries({
								ids : paidTranIds
							}));
					paidTranEntries = cashflowHelper
							.assembleTransactionEntries(paidEntries);
					log.debug('AR: returned raw Paid results', paidTranEntries);
				}

				// Credit
				if (commons.makesure(params[1])) {
					creditTranEntries = helper.resultset(cashflowDAO
							.fetchTransactionEntries({
								ids : params[1],
								types : [ 'CustCred'/*
													 * {value:'CustCred',
					...
					log.debug('AR: returned raw Credit results',
							creditTranEntries);
				}
			}
			...
			function collectAR() {
				...
				if (tranEntries[0].getValue('type') === 'CustPymt') {
					if (cashflowHelper.isDepositedPayment({
						...
						collectDepositedFunds(); // paid funds of invoices
						// and Statement charge
						collectCreditedFunds(); // credited funds of paid
						// invoices
						collectCredits(); // credits
					}
				} else if (tranEntries[0].getValue('type') === 'CashSale'
						|| tranEntries[0].getValue('type') === 'CashRfnd'
						|| tranEntries[0].getValue('type') === 'CustRfnd') {
					if (cashflowHelper.isDepositedPayment({
						tranEntries : tranEntries,
						depositTranEntries : depositTranEntries
					})) {
			...
			function collectDepositedCash() {
				...
				for (var i = 0; i < tranEntries.length; i++) {
					if (!cashflowHelper.isMainline(tranEntries[i])) {
						...
						doCollect({
							depositTran : commons.makesure(depositTranEntry) ? depositTranEntry.id
									: null,
							...
							// NSCHINA-2429
							// Add location/department/classification filter to
							// CFS AR Deposited Cash collect
			...
			/*
			 * @desc Collect customer deposit entries. For now, it includes: 1.
			 * account deposit 2. undepfund Later, invoice will be added.
			 */
				...
				for (var i = 0; i < tranEntries.length; i++) {
					if (!cashflowHelper.isMainline(tranEntries[i])) {
						doCollect({
							...
							// NSCHINA-2429
							// Add location/department/classification filter to
							// CFS AR Customer Deposits Collect
							location : getLocation(tranEntries[i]),
							department : getDepartment(tranEntries[i]),
							classification : getClassification(tranEntries[i])
						});
				...
				for (var k = 0; k < tranEntries.length; k++) {
					...
					for (var i = 0; i < invTranEntries.length; i++) {
						if (!cashflowHelper.hasSameAccountType({
							...
							doCollect({
								depositTran : commons
										.makesure(depositTranEntry) ? depositTranEntry.id
										: null,
								...
								cashamount : depositedAmount,
								// NSCHINA-2429
								// Add location/department/class to CFS AR
								// Deposited Funds Collect
			...
			function collectCredits() {
				...
				for (var k = 0; k < creditTranEntries.length; k++) {
					...
					if (!commons.makesure(invTranId)
							|| !hasPaidTransactionEntries(invTranId)) {
						continue;
					}

					// This is because credit memo amount get accumulated on the
					// same credit memo so we have to delete previously
					// collected entries and re-collect.
					collectoDAO.deleteConditionedCollectedEntries({
						lineUniqueKey : creditTranEntries[k].id + '-'
								+ invTranId
					});

					var creTranEntries = creditTransactionEntries(creditTranEntries[k].id);
					var inline = cashflowHelper
							.hasInlineCashflowItem(creTranEntries);
					for (var i = 0; i < creTranEntries.length; i++) {
						if (!cashflowHelper.hasSameAccountType({
							...
							doCollect({
								...
								cashamount : -depositedAmount,
								// NSCHINA-2429
								// Add location/department/classification filter
								// to CFS AR Credit collect
			...
			function collectCreditedFunds() {
				...
				for (var k = 0; k < creditTranEntries.length; k++) {
					...
					// This is because credit memo amount get accumulated on the
					// same invoice so we have to delete previously collected
					// entries and re-collect.
					collectoDAO.deleteConditionedCollectedEntries({
						lineUniqueKey : creditTranEntries[k].id + '-'
								+ invTranId
					});
					...
					invTranEntries = removeEntries(invTranEntries);
					var inline = cashflowHelper
							.hasInlineCashflowItem(invTranEntries);
					for (var i = 0; i < invTranEntries.length; i++) {
			...
			function subsidiary(entry) {
				if (commons.makesure(depositTranEntries)) {
					return cashflowHelper
							.getCurrentSubsidiary(depositTranEntries[0]
									.getValue('subsidiary'));
				} else {
					return cashflowHelper.getCurrentSubsidiary(entry
							.getValue('subsidiary'));
				}
			}
			...
			function depositTransactionEntry(appliedToEntry) {
				...
				if (!commons.makesure(value)) {
					for (var i = 0; i < depositTranEntries.length; i++) {
						var appliedTranId = depositTranEntries[i]
								.getValue('appliedToTransaction');
				...
				for ( var i in entries) {
					...
					if (entryType === 'ShipItem') {
						shippingAmount = shippingAmount
								+ commons.toNumber(entries[i]
										.getValue('netamount'))
								+ commons.toNumber(entries[i]
										.getValue('taxamount'));
					} else if (entryType === 'GiftCert'
							&& !commons.makesure(entries[i]
									.getValue('taxamount'))) {
						// gift do not has taxamount, gift as a item need to be
						// collected and has taxamount
			...
			/*
			 * @desc Return paid transaction entries like invoice, bill. Query
			 * from record if desn't exist for case paid transaction and payment
			 * in different date. @param {String} key - Internal ID of paid
			 * transaction entries @return {search.Result[]}
			 */
			...
			function creditTransactionEntries(key) { // JL - nice to enhance
				// with a map
				var entries = [];
			...
			/*
			 * @desc Calculate and return deposited cash amount. @param {Object}
			 * params @params {search.Result} params.payingTranEntry - The
			 * transaction entry that really has cash-in-out @params
			 * {search.Result} params.paidTranEntry - The transaction entry on
			 * which payment should be done @return {Number}
			 */
			function depositedCashAmount(params) {
				...
				var tranEntry = commons.makesure(paidTranEntry) ? paidTranEntry
						: payingTranEntry;
				...
				if (commons.ensure(params.isLastItem)) {
					var amount = format.round(commons.toAbsNumber(paidAmount)
							- commons.toAbsNumber(precedingLinePaidAmount));
					precedingLinePaidAmount = 0;
				} else {
					amount = format.round((lnTotalAmount * paidAmount)
							/ lnsTotalAmount(tranEntry));
				...
				if (commons.makesure(depositTranEntry)) {
					if (commons.makesure(paidTranEntry)) {
						if (payingTranEntry === tranEntries[tranEntries.length - 1]) {
							var paidAmount = commons
									.toAbsNumber(depositTranEntry
											.getValue('amount'))
									- commons
											.toAbsNumber(precedingTransPaidAmount);
							if (commons.ensure(params.isLastItem)) {
								returnPaidAmountToZero();
							}
						} else {
							paidAmount = (payingTranEntry
									.getValue('paidamount') * depositTranEntry
									.getValue('amount'))
									/ payingTranEntry.getValue('total');
					...
					if (commons.makesure(paidTranEntry)) {
						if (runtime.isMultiCurrency()
								&& payingTranEntry.getValue('type') === 'CustCred') {
							paidAmount = payingTranEntry
									.getValue('appliedtoforeignamount')
									* tranEntries[0].getValue('exchangerate');
						} else {
			...
			function lnsTotalAmount(tranEntry) {
				...
				return commons.toNormalizedNumber(totalAmount)
						- commons.toNormalizedNumber(otherExpenses);
			}
			...
			function collectDeposits() {
				...
				for (var i = 0; i < depositedTranEntries.length; i++) {
					...
					if (tranEntries.length === 0
							|| entry.id === tranEntries[0].id) { // JL - can
						// enhance
						// with
						// assembling
						// in
			...
			function matches(tranType) {
				var recordTypeWhiteList = [ record.Type.CUSTOMER_PAYMENT,
						record.Type.DEPOSIT, record.Type.CUSTOMER_DEPOSIT,
						record.Type.CUSTOMER_REFUND, record.Type.CASH_SALE,
						record.Type.CASH_REFUND, 'CustPymt', 'Deposit',
						'CustDep', 'CustRfnd', 'CashSale', 'CashRfnd' ];
