...
define(
		...
		function(record, commons, file, runtime, formatter, search) {
			...
			function fetchPymtApplyInfo(pymtRecord, param, JERec, JETransDate,
				...
				var cleared = pymtRecord.getValue('cleared');
				...
				if (acctType.type[0].value == 'CredCard') {
					if (cleared == 'F') {
						return pymAppliedItems;
					}
				}
				...
				if (entityCategory == '16') {// 如果category为employee则需要回查
					...
					item.cfi = payCfi;
					var _amount = total * exchangeRate * param;
					item.amount = _amount.toFixed(2);
					lineInfos.push(item);
					pymAppliedItems[JERec.id] = lineInfos;
				}
			...
			function fetchJEApplyInfo(pymtRecord) {
				...
				var period = pymtRecord.getValue('postingperiod');
				var subsidiary = pymtRecord.getValue('subsidiary');
				// 如果是Voiding Journal 则调用bill payment现金流量表方法
				var createdFrom = pymtRecord.getValue('createdfrom');
				if (createdFrom) {
					var billPymtRecord = record.load({
						type : 'vendorpayment',
						id : createdFrom
					});
					var pymAppliedItemsP = fetchPymtApplyInfo(billPymtRecord,
							-1, pymtRecord, transDate, period);
			...
			function fetchDepApplyInfo(pymtRecord) {
				...
				// });
				// for (var i = 0; i < accBokCount; i++) {
				// accountingbook这个字段取不到值，应该是系统bug，故只能取第一条exchangerate
