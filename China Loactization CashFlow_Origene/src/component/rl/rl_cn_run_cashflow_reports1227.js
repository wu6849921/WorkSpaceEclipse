 ...
 * 
 * For China Cash Flow Statement solution, need read below values from standard
 * reports. Effective of foreign exchange rate changes on cash and cash
 * equivalents Beginning balance of cash and cash equivalents Ending balance of
 * cash and cash equivalents
 * 
 * And rl_cn_run_cashflow_reports is used to run those reports using
 * nlapiRunReport API.
 * 
 * Params subsidiary - mandatory for One-World account, will throw error if it
 * is One-World account and subsidiary is not given fiscalCalendar - mandatory
 * if it is One-World account and Multi-Calendar feature has been enabled
 * periodFrom - mandatory periodTo - mandatory location - optional department -
 * optional classification - optional
 * 
 * Return String representative of below json object. {
 * unrealizedGainAndLossCurrent: 0, unrealizedGainAndLossPrior: 0,
 * startBalanceCurrent: 0, startBalancePrior: 0, endBalanceCurrent: 0,
 * endBalancePrior: 0 }
 */
if (!cn) {
	var cn = {};
...
cn.rlv1.fin.standard.reports = function() {
	...
	this.doGet = function(params) {
		...
		var periodObj = getPeriod(paramsObj.periodFrom, paramsObj.periodTo,
				paramsObj.fiscalCalendar);
		...
		// If multi-currency does not enabled, then there won't be any
		// unrealized exchange rate gains and losses
		if (isMultiCurrencyEnabled()) {
			// line 35, Effective of foreign exchange rate changes on cash and
			...
			// line 35, Effective of foreign exchange rate changes on cash and
			// cash equivalents - Prior Period
			// It might happen that periodFrom.samePeriodLastFiscalYear does not
			// exist, but periodTo.samePeriodLastFiscalYear exists
			if (periodObj.periodTo.samePeriodLastFiscalYear.id) {
				if (periodObj.periodFrom.samePeriodLastFiscalYear.id) {
					...
					var from = getEarliestPeriod(
							fiscalCalendar,
							periodObj.periodFrom.samePeriodLastFiscalYear.startdate,
							periodObj.periodTo.samePeriodLastFiscalYear.startdate);
		...
		// line 37, Beginning balance of cash and cash equivalents - Current
		// Period
		...
		// line 37, Beginning balance of cash and cash equivalents - Prior
		// Period
	...
	/**
	 * Validate passed-in parameters. Will throw exception if meets below
	 * condition: passed-in parameters is undefined, null or '' One-World
	 * account and subsidiary is undefined, null or '' One-World account with
	 * Multi-Calendar feature been enabled and fiscalCalendar is undefined, null
	 * or '' periodFrom is undefined, null or '' periodTo is undefined, null or ''
	 * 
	 * @return object of pass in parameters if there are no exception
	 */
	function validateParams(params) {
		if (!params) {
			throw nlapiCreateError('RunStandardReportError',
					'Run report params are undefined', true);
		}
		nlapiLogExecution('AUDIT', 'validateParams', 'params='
		...
		if (isOW && !paramsObj.subsidiary) {
			throw nlapiCreateError('RunStandardReportError',
					'Subsidiary is mandatory for OW account', true);
		}

		if (isOW && isMultiCalendarEnabled() && !paramsObj.fiscalCalendar) {
			throw nlapiCreateError(
					'RunStandardReportError',
					'fiscalCalendar is mandatory for OW account with Multi-Calendar enabled',
					true);
		}
		...
		if (!paramsObj.periodFrom) {
			throw nlapiCreateError('RunStandardReportError',
					'period from is mandatory', true);
		}

		if (!paramsObj.periodTo) {
			throw nlapiCreateError('RunStandardReportError',
					'period to is mandatory', true);
		}
	...
	function runGeneralLedgerReport(paramsObj) {
		nlapiLogExecution('AUDIT', 'runGeneralLedgerReport', 'paramsObj='
				+ JSON.stringify(paramsObj));
		return runStandardReport(293,
				composeGeneralLedgerReportSettings(paramsObj),
		...
		try {
			...
			for ( var colIdx in colChildren) {
				nlapiLogExecution('DEBUG', 'runStandardReport',
						'target col label: ' + colChildren[colIdx].getLabel());
			...
			if (targetCol === null) {
				throw nlapiCreateError('RunStandardReportError', 'Column '
						+ targetColumnName + ' does not exists', true);
			}
	...
	function composeGeneralLedgerReportSettings(paramsObj) {
		...
		if (paramsObj.location) {
			reportSettings.addCriteria(
					'regtx,tranline,klocation,x,alltranlineloc',
					paramsObj.location);
		}
		if (paramsObj.department) {
			reportSettings.addCriteria(
					'regtx,tranline,kdepartment,x,alltranline29',
					paramsObj.department);
		}
		if (paramsObj.classification) {
			reportSettings.addCriteria('regtx,tranline,kclass,x,alltranline6',
					paramsObj.classification);
		}
		return reportSettings;
	}

	function runUnrealizedGainsAndLossesReport(paramsObj) {
		nlapiLogExecution('AUDIT', 'runUnrealizedGainsAndLossesReport',
				'paramsObj=' + JSON.stringify(paramsObj));
		return runStandardReport(-133,
				composeUnrealizedGainsAndLossesReportSettings(paramsObj),
				unrealizedGainAndLossColumnName);
	}
	...
	function composeUnrealizedGainsAndLossesReportSettings(paramsObj) {
		...
		reportSettings.addCriteria(
				'urg,x,acct_saccttype,tranline,alltranline5', 'Bank');
		if (paramsObj.location) {
			reportSettings.addCriteria(
					'urg,tranline,klocation,x,alltranlineloc',
					paramsObj.location);
		}
		if (paramsObj.department) {
			reportSettings.addCriteria(
					'urg,x,dpt_sname,tranline,alltranline29',
					paramsObj.department);
		}
		if (paramsObj.classification) {
			reportSettings.addCriteria('urg,x,cls_sname,tranline,alltranline6',
					paramsObj.classification);
		}
		return reportSettings;
	}
	...
	function composeReportSettings(paramsObj) {
		var settings = new nlobjReportSettings(paramsObj.periodFrom,
				paramsObj.periodTo);
		if (paramsObj.subsidiary) {
			settings.setSubsidiary(paramsObj.subsidiary);
		}
		// settings.setAccountingBook('5');
		// nlapiLogExecution('DEBUG', 'settings', 'settings = '
	...
	function setupUserPref() {
		...
		nlapiLogExecution('AUDIT', 'setupUserPref', 'reportByPeriod = '
				+ this.reportByPeriod);
	...
	/**
	 * Get accounting period information. If periodFrom is 'Jan 2018', periodTo
	 * is 'Mar 2018', then return { 'periodFrom': { 'lastPeriod': { 'startdate':
	 * '12/1/2017', 'id': <internalid of 'Dec 2017' or undefined>, 'name': <'Dec
	 * 2017' or undefined> }, 'samePeriodLastFiscalYear': { 'startdate':
	 * '1/1/2017', 'id': <internalid of 'Jan 2017' or undefined>, 'name': <'Jan
	 * 2017' or undefined> }, 'lastPeriodOneFiscalYearAgo': { 'startdate':
	 * '12/1/2016', 'id': <internalid of 'Dec 2016' or undefined>, 'name': <'Dec
	 * 2016' or undefined> } }, 'periodTo': { 'lastPeriod': { 'startdate':
	 * '2/1/2018', 'id': <internalid of 'Feb 2018' or undefined>, 'name': <'Feb
	 * 2018' or undefined> }, 'samePeriodLastFiscalYear': { 'startdate':
	 * '3/1/2017', 'id': <internalid of 'Mar 2017' or undefined>, 'name': <'Mar
	 * 2017' or undefined> }, 'lastPeriodOneFiscalYearAgo': { 'startdate':
	 * '2/1/2017', 'id': <internalid of 'Feb 2017' or undefined>, 'name': <'Feb
	 * 2017' or undefined> } } }
	 */
	function getPeriod(periodFrom, periodTo, fiscalCalendar) {
		...
		if (fiscalCalendar) {
			...
			filterExpression.push([ "fiscalcalendar", "is", fiscalCalendar ]);
		}

		var periodNameCol = new nlobjSearchColumn('periodname');
		var startDateCol = new nlobjSearchColumn('startdate');
		var columns = [ periodNameCol, startDateCol ];
		var searchresults = nlapiSearchRecord('accountingperiod', null,
				filterExpression, columns);
		nlapiLogExecution('DEBUG', 'getPeriod', 'searchresults = '
		...
		nlapiLogExecution('AUDIT', 'getPeriod', 'periodObj = '
				+ JSON.stringify(rtn));
		return rtn;
	}

	function getStartDateInfo(periodFrom, periodTo) {
		var filterExpression = [ [ "internalid", "anyof", periodFrom, periodTo ] ];

		var periodNameCol = new nlobjSearchColumn('periodname');
		var lastPeriodCol = new nlobjSearchColumn('formuladate')
				.setFormula("ADD_MONTHS({startdate}, -1)");
		var samePeriodLastFiscalYearCol = new nlobjSearchColumn('formuladate')
				.setFormula("ADD_MONTHS({startdate}, -12)");
		var lastPeriodOneFiscalYearAgoCol = new nlobjSearchColumn('formuladate')
				.setFormula("ADD_MONTHS({startdate}, -13)");

		var columns = [ periodNameCol, lastPeriodCol,
				samePeriodLastFiscalYearCol, lastPeriodOneFiscalYearAgoCol ];
		...
		for (var i = 0; searchresults != null && i < searchresults.length; i++) {
			...
			if (id === periodFrom) {
				var fromPeriodObj = {
					...
					'samePeriodLastFiscalYear' : searchresult
							.getValue(samePeriodLastFiscalYearCol),
					'lastPeriodOneFiscalYearAgo' : searchresult
							.getValue(lastPeriodOneFiscalYearAgoCol)
				};
			}
		...
		nlapiLogExecution('DEBUG', 'getStartDateInfo', 'fromPeriodObj='
				+ JSON.stringify(fromPeriodObj));
		nlapiLogExecution('DEBUG', 'getStartDateInfo', 'toPeriodObj='
				+ JSON.stringify(toPeriodObj));
		return {
	...
	/**
	 * Get earliest period that startdate is within startdateFrom and
	 * startdateTo.
	 */
	function getEarliestPeriod(fiscalCalendar, startdateFrom, startdateTo) {
		nlapiLogExecution('DEBUG', 'getEarliestPeriod', 'fiscalCalendar='
				+ fiscalCalendar + ', startdateFrom=' + startdateFrom
				+ ', startdateTo=' + startdateTo);
		var filters = getBasePeriodFilter(fiscalCalander);
		filters.push("AND").push(
				[ "startdate", "within", startdateFrom, startdateTo ]);
		nlapiLogExecution('DEBUG', 'getEarliestPeriod', 'filters='
		...
		if (!searchresults) {
			throw nlapiCreateError('RunStandardReportError',
					'Should have at least one period between ' + startdateFrom
							+ " and " + startdateTo, true);
		}
		nlapiLogExecution('DEBUG', 'getEarliestPeriod', 'return='
				+ searchresults[0].getValue(periodNameCol));
		return searchresults[0].id;
	}

	function getBasePeriodFilter(fiscalCalendar) {
		var baseFilter = [ [ "isadjust", "is", "F" ], "AND",
				[ "isinactive", "is", "F" ], "AND", [ "isquarter", "is", "F" ],
				"AND", [ "isyear", "is", "F" ] ];
		if (fiscalCalander) {
	...
	function getUserLanguage() {
		return nlapiLoadConfiguration('userpreferences').getFieldValue(
				'language');
	}
	...
	function isObject(bechecked) {
		return typeof bechecked === 'object'
				&& bechecked.constructor === Object
				|| Object.prototype.toString.apply(bechecked) === '[object Object]';
	}
};
