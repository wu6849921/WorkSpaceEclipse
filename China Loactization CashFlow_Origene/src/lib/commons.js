...
define(
		...
		function() {
			...
			function isString(bechecked) {
				return typeof bechecked === 'string'
						&& bechecked.constructor === String
						|| Object.prototype.toString.apply(bechecked) === '[object String]'
						|| bechecked instanceof String;
			}

			function isArray(bechecked) {
				return bechecked
						&& typeof bechecked === 'object'
						&& bechecked.constructor === Array
						|| Object.prototype.toString.apply(bechecked) === '[object Array]'
						|| bechecked instanceof Array;
			}

			function isBoolean(bechecked) {
				return typeof bechecked === 'boolean'
						&& bechecked.constructor === Boolean
						|| Object.prototype.toString.apply(bechecked) === '[object Boolean]'
						|| bechecked instanceof Boolean;
			}
			...
			function isNumber(bechecked) {
				...
				return (typeof bechecked === 'number'
						&& bechecked.constructor === Number
						|| Object.prototype.toString.apply(bechecked) === '[object Number]' || bechecked instanceof Number)
						&& isFinite(bechecked);
			}

			function isNaN(bechecked) {
				return String(bechecked) === 'NaN'; // Don't use JavaScript
													// isNaN as isNaN('a')
													// returns true.
													// Number.isNaN is
													// introduced in ES6.
			...
			function isDate(bechecked) {
				return (typeof bechecked === 'date'
						&& bechecked.constructor === Date
						|| Object.prototype.toString.apply(bechecked) === '[object Date]' || bechecked instanceof Date)
						&& !isNaN(bechecked.valueOf());
			}

			function isFunction(bechecked) {
				return typeof bechecked === 'function'
						&& bechecked.constructor === Function
						|| Object.prototype.toString.apply(bechecked) === '[object Function]'
						|| bechecked instanceof Function;
			}

			function isObject(bechecked) {
				return bechecked
						&& (typeof bechecked === 'object'
								&& bechecked.constructor === Object || Object.prototype.toString
								.apply(bechecked) === '[object Object]');
			}

			function isPrimitive(bechecked) {
				return isString(bechecked) || isBoolean(bechecked)
						|| isNumber(bechecked);
			}
			 ...
			 * 
			 * @param {object}
			 *            bechecked target object to be tested
			...
			/**
			 * This function behave differently on checking 0, false, blank
			 * string with only spaces than the
			 * 
			 * @function ensure.
			 * 
			 * E.g. makesure({}) -> true makesure([]) -> false makesure('') ->
			 * false makesure(' ') -> false makesure(null) -> false
			 * makesure(undefined) -> false makesure(NaN) -> false
			 * 
			 * @param {object}
			 *            bechecked target object to be tested
			 * @returns true if the target object is valid, false otherwise.
			...
			/**
			 * This function behave differently on checking empty object like {}
			 * than the
			 * 
			 * @function makesure.
			 * 
			 * E.g. force({}) -> false force([]) -> false force('') -> false
			 * force(' ') -> false force(null) -> false force(undefined) ->
			 * false force(NaN) -> false
			 * 
			 * @param {object}
			 *            bechecked target object to be tested
			 * @returns true if the target object is valid, false otherwise.
			...
			function isEmptyObject(bechecked) {
				...
				// enumerable properties. If you want all properties, even not
				// enumerables, use Object.getOwnPropertyNames.
			...
			/**
			 * Copy values from NetSuite record.Record object or NetSuite
			 * search.Result object to JavaScript object identified by property
			 * names. This method doesn't support the join for the search return
			 * column.
			 * 
			 * @param {object}
			 *            from Any NetSuite record.Record or search.Result
			 *            object.
			 * @param {object}
			 *            to Any JavaScript object including array.
			 * @param {array}
			 *            names An array contains objects like {name: 'name',
			 *            type: 'text'}, type can be 'text' and 'value'.
			 * @returns
			 */
			...
			/**
			 * Check if target object(JavScript object, NetSuite record.Record
			 * and NetSuite search.Result) has valid specified field.
			 * 
			 * @param target
			 *            The target object to be tested.
			 * @param field
			 *            The specified field for which to check validity.
			 * @returns true if the target object has a valid specified field,
			 *          false otherwise.
			 */
			...
			function replace(bereplaced, occurrence, replacement) {
				if (!isString(bereplaced) || !isString(occurrence)
			...
			function sign(bechecked) {
				if (bechecked === null || !isNumber(toNumber(bechecked))) {
					throw new TypeError(bechecked
							+ ' is not a normalized number');
				}
			 ...
			 * 
			 * @param {Array}
			 *            becheckedArray The being checked array
			 * @param {Object |
			 *            Primitive} targetValue The target value
			 * @return true if checked array contains the target value, false
			 *         otherwise
			 */
			function contains(becheckedArray, targetValue) {
				...
				while (index--) {
					if (isObject(targetValue)) {
						if (JSON.stringify(becheckedArray[index]) === JSON
			...
			/**
			 * @desc This function will keep your script busy for some seconds.
			 *       This is a simulation of so-called sleep function.
			 * @param {Number}
			 *            seconds - The time in seconds to sleep
			 */
			...
			/**
			 * Clone the given object and generate a new one. Deep clone has its
			 * limit of only working on JSON-serializable content.
			 * 
			 * @param {*}
			 *            beCloned - The object to be cloned
			 * @param {Boolean}
			 *            deep - Identify if deep clone
			...
			function clone(beCloned, deep) {
				...
				if (isObject(beCloned)) {
					if (!deep) {
						throw "Not implemented"; // Object.assign({},
													// beCloned); // ES6
					} else {
						return JSON.parse(JSON.stringify(beCloned));
					}
			 ...
			 * @param {Funtion}
			 *            mapper map entry to a string for identifying equality
			 *            of entries if no mapper, use original entry
