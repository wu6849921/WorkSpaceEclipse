/*******************************************************************************
 * File Name : TN_UE_PurchaseOrder.js Description : User Event on purchase order
 * Company : Trigger Networks Created By : Daniel Cai Created On : 04/11/2014
 ******************************************************************************/
...
function poBeforeLoad(type, form, request) {
	try {
		...
		if (poApprovalStatus.indexOf('Rejected') != -1) {
			form.setScript('customscript_tn_ueonpo');
			if (type == 'view')
				form.addButton('custpage_rejectreason', 'Reject Reason',
						'rejectReason()');
		}
		if (type == 'create') {
			...
			var statusFld = nlapiGetField('custbody_tn_prstatus')
			...
			for (var i = 1; i <= oldPOCount; i++) {
				nlapiSetLineItemValue('item', 'item', i, oldPO
						.getLineItemValue('item', 'item', i));
				nlapiSetLineItemValue('item', 'quantity', i, oldPO
						.getLineItemValue('item', 'quantity', i));
				nlapiSetLineItemValue('item', 'description', i, oldPO
						.getLineItemValue('item', 'description', i));
				nlapiSetLineItemValue('item', 'custcol_tn_prpurpose', i, oldPO
						.getLineItemValue('item', 'custcol_tn_prpurpose', i));
				nlapiSetLineItemValue('item', 'rate', i, oldPO
						.getLineItemValue('item', 'rate', i));
				nlapiSetLineItemValue('item', 'custcol_tn_unitpriceincltax', i,
						oldPO.getLineItemValue('item',
								'custcol_tn_unitpriceincltax', i));
				nlapiSetLineItemValue('item', 'amount', i, oldPO
						.getLineItemValue('item', 'amount', i));
				nlapiSetLineItemValue('item', 'taxcode', i, oldPO
						.getLineItemValue('item', 'taxcode', i));
			}
		}
		...
		if (type == 'view' && (roleId == 3 || roleId == 1005 || roleId == 1007)) {
			...
			// nlapiLogExecution('debug', 'test', 'poStatus: ' + poStatus + ',
			// poApprovalStatus: ' + poApprovalStatus);
			/*
			 * if (poStatus == 'Approved by Supervisor/Pending Receipt' ||
			 * poStatus == 'Approved by Supervisor/Pending Bill' || poStatus ==
			 * 'Pending Billing/Partially Received') {
			 * form.addButton('custpage_closepo', 'Close', 'closePO()'); }
			 */
			form.addButton('custpage_closepo', 'Close', 'closePO()');
			if (poApprovalStatus == 'Open') {
				var noticeFld = form.addField('custpage_notice', 'inlinehtml',
						'Notice', null, null).setLayoutType('outsideabove',
						'startcol');
		...
		if (type == 'view'
				&& (roleId == 3 || roleId == 1000 || roleId == 1013
						|| roleId == 1014 || roleId == 1005)) {
		...
		if (type != 'create') {
			...
			columns.push(new nlobjSearchColumn(
					'custbody_tn_vendorcontractexpire'));
			var poResult = nlapiSearchRecord('purchaseorder', null, filters,
					columns);
			var amount = poResult[0].getValue('amount');
			if (amount <= 30000) {
				nlapiGetField('custbody_tn_cfoapproved').setDisplayType(
						'hidden');
				nlapiGetField('custbody_tn_cfoadate').setDisplayType('hidden');
			}

			if (amount < 2000000
					|| poResult[0].getValue('custbody_tn_contractexperid') != 'T') {
				// nlapiLogExecution('debug', 'Hide Fields', amount + ', ' +
				// poResult[0].getValue('custbody_tn_contractexperid'));
				nlapiGetField('custbody_tn_financeapprove').setDisplayType(
						'hidden');
				nlapiGetField('custbody_tn_fadate').setDisplayType('hidden');
				nlapiGetField('custbody_tn_legalapproved').setDisplayType(
						'hidden');
				nlapiGetField('custbody_tn_ladate').setDisplayType('hidden');
			} else {
				// nlapiLogExecution('debug', 'Show Fields', amount + ', ' +
				// poResult[0].getValue('custbody_tn_contractexperid'));
				nlapiGetField('custbody_tn_financeapprove').setDisplayType(
						'inline');
...
function makeCopy() {
	try {
		// #region Suitelet
		/*
		 * var poId = nlapiGetRecordId(); var scriptId =
		 * 'customscript_tn_copypoinsuitelet'; var deployId =
		 * 'customdeploy_tn_copypoinsuitelet'; var paramStr = '&poId=' + poId;
		 * var myurl = nlapiResolveURL('SUITELET', scriptId, deployId) +
		 * paramStr; window.open(myurl, "_blank");
		 */
		// #endregion
		// #region User Event
		var poId = nlapiGetRecordId();
		var paramStr = '&poId=' + poId;
		var preHref = window.location.href.substring(0, window.location.href
				.indexOf('.com/') + 5);
		var myurl = preHref
				+ 'app/accounting/transactions/purchord.nl?cf=102&poId=' + poId
				+ '&whence=';
...
function poAfterSubmit(type) {
	try {
		...
		var poId = nlapiGetRecordId();
		var roleId = nlapiGetRole();
		var needPopup = nlapiLookupField('purchaseorder', poId,
				'custbody_tn_popup5kform');
		...
		var filters = new Array();
		filters.push(new nlobjSearchFilter('custrecord_tn_prid', null, 'is',
				poId));
		// filters.push(new nlobjSearchFilter('custrecord_tn_formcompleted',
		// null, 'is', 'T'));
		var columns = new Array();
		columns.push(new nlobjSearchColumn('custrecord_tn_formcompleted'));
		var cfoForm = nlapiSearchRecord('customrecord_tn_cfoform', null,
				filters, columns);
		// if (isNull(cfoForm)) {
		// // alert('Please complete the International Requisition Form');
		// nlapiSetFieldValue('custbody_tn_prchecked', 'F');
		// if (needPopup == 'T')
		// nlapiSetRedirectURL('RECORD', 'customrecord_tn_cfoform', null,
		// null, params);
		// } else {
		// var isCompleted = cfoForm[0]
		// .getValue('custrecord_tn_formcompleted');
		// if (needPopup == 'T' && isCompleted != 'T')
		// nlapiSetRedirectURL('RECORD', 'customrecord_tn_cfoform',
		// cfoForm[0].getId());
		// }
		// if (needPopup == 'T' && (roleId == '1000' || roleId == '1013' ||
		// roleId == '1014' || roleId == '3'))
		// nlapiSetRedirectURL('RECORD', 'customrecord_tn_cfoform', null, null,
		// params);
...
function sendEmail(type) {
	try {
		...
		var roleId = nlapiGetRole();
		// nlapiLogExecution('debug', 'sendEmail', 'businessType: ' +
		// businessType + ', ' + 'statusId: ' + statusId);
		var ccer = null;
		var ccer2 = null;
		switch (statusId) {
		case '6':
			emailTitle1 = isResubmit == 'T' ? 'PO#' + poNumber
					+ ' Has Been Re-Submited' : 'PO#' + poNumber
					+ ' Has Been Submited';
		...
		case '1':
			...
			sender = currentUserId;
			receiver = employeeId;
			var createdByEmail = nlapiLookupField('employee', createdBy,
					'email');
		...
		case '12':
			emailTitle1 = 'PO#'
					+ poNumber
					+ '. Reply "confirmed" if you\'ve confirmed the goods/service receipt. Purchaser could continue the payment process';
			sender = currentUserId;
			receiver = employeeId;
		...
		case '18':
			...
			if (roleId == 1008)
				approverRole = ' by CFO';
			emailTitle1 = cfoApproved == 'T' ? 'PO#' + poNumber
					+ ' Has Been Approved by CFO' : 'PO#' + poNumber
					+ ' Has Been Approved' + approverRole;
			sender = currentUserId;
			receiver = employeeId;
			ccer = nlapiLookupField('employee', createdBy, 'email');
			emailTitle2 = 'PO#' + poNumber
					+ '. A Contract Needs to be Approved';
			otherId = nextApproverId;
			ccer2 = 'achen@realnetworks.com';
			nlapiLogExecution('debug', 'test', 'executed: 18' + ', otherId: '
					+ nextApproverId);
			break;
		case '7':
		...
		nlapiLogExecution('debug', 'sendEmail', 'Email Sending, ' + sender
				+ ', ' + receiver + ', ' + otherId + ', ' + ccer + ', '
				+ emailTitle1 + ', ' + emailTitle2 + ' .');
		...
		htmlStr += '<p>Requester:&nbsp;&nbsp;' + poRec.getFieldText('employee')
				+ '</p>';
		htmlStr += '<p>Division Code:&nbsp;&nbsp;'
				+ poRec.getFieldValue('custbody_tn_divisioncodepr') + '</p>';
		htmlStr += '<p>Location:&nbsp;&nbsp;' + poRec.getFieldText('location')
				+ '</p>';
		htmlStr += '<p>Vendor Name:&nbsp;&nbsp;' + poRec.getFieldText('entity')
				+ '</p>';
		htmlStr += '<p>Currency:&nbsp;&nbsp;' + poRec.getFieldText('currency')
				+ '</p>';
		var preHref = nlapiResolveURL('SUITELET',
				'customscript_tn_sl_datacenterdetector',
				'customdeploy_tn_sl_datacenterdetector', true);
		preHref = preHref.replace('forms', 'system');
		preHref = preHref.substring(0, preHref.indexOf('.com/') + 4);
		var poUrl = preHref
				+ nlapiResolveURL('RECORD', 'purchaseorder', poId, 'VIEW');
		htmlStr += '<p>PO No.:&nbsp;&nbsp;' + poRec.getFieldValue('tranid')
				+ ',&nbsp;&nbsp;<a href="' + poUrl + '">View Record</a></p>';
		htmlStr += '<p>PO Amount:&nbsp;&nbsp;' + poRec.getFieldValue('total')
				+ '</p>';
		...
		for (var i = 1; i <= poRec.getLineItemCount('item'); i++) {
			htmlStr += '<tr><td>' + i + '</td>';
			htmlStr += '<td>' + poRec.getLineItemText('item', 'item', i)
					+ '</td>';
			htmlStr += '<td style="text-align: right">'
					+ parseFloat(poRec.getLineItemValue('item', 'quantity', i))
							.toFixed(2) + '</td>';
			htmlStr += '<td>'
					+ convertNull(poRec.getLineItemValue('item', 'description',
							i)) + '</td>';
			...
			var grossAmt = (poRec.getLineItemValue('item', 'amount', i) * (1 + taxRate))
					.toFixed(2);
		...
		if (isNull(sender))
			...
			if (!isNull(receiver) && !isNull(emailTitle1)) {
				...
				nlapiLogExecution('debug', 'test', 'receiver: ' + receiver
						+ ', sender: ' + sender + ', otherId: ' + otherId);
				// ccer = null; // this line will be deleted in UAT
				if (isNull(ccer))
					nlapiSendEmail(sender, receiver, emailTitle1, htmlStr,
							null, null, records, null, null, false);
				else
					nlapiSendEmail(sender, receiver, emailTitle1, htmlStr,
							ccer, null, records, null, null, false);
				nlapiLogExecution('debug', 'test', '1 email sent');
			}
			if (!isNull(otherId) && !isNull(emailTitle2)) {
				otherId = otherId == 152 ? 176 : otherId;
				if (isNull(ccer2))
					nlapiSendEmail(sender, otherId, emailTitle2, htmlStr, null,
							null, records, null, null, false);
				else
					nlapiSendEmail(sender, otherId, emailTitle2, htmlStr,
							ccer2, null, records, null, null, false);
				nlapiLogExecution('debug', 'test', '2 email sent');
			}
			if (nextApproverId == 131 || nextApproverId == 114) {
				nlapiSendEmail(sender, 'ckarusala@realnetworks.com', 'PO# '
						+ poNumber + ' is in approval', htmlStr, null, null,
						records, null, null, false);
	...
	try {
		...
		window
				.open(
						myurl,
						"newwindow",
						"height=300px, width=600px, top=0, left=0,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
	} catch (ex) {
		alert('Error');
		alert(ex);
		alert(ex.getDetails());
