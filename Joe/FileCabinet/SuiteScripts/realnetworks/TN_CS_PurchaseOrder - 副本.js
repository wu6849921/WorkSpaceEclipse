	...
	try {
		...
		if ((currency.indexOf('RMB') != -1 && totalAmt > 30000)
			...
			if (statusId == 9) { // Open
				...
				if (isSubmit == 'T') {
					alert('The total amount is over 5KU, please fill out the additional information. Thank you!');
					nlapiSetFieldValue('custbody_tn_complete', 'F');
					nlapiSetFieldValue('custbody_tn_popup5kform', 'T');
				} else
					alert('Your purchase request is NOT submitted. Current informationwill be saved as a draft.');
				return true;
			} else {
				...
				filters.push(new nlobjSearchFilter(
						'custrecord_tn_formcompleted', null, 'is', 'T'));
			...
			var isSubmit = nlapiGetFieldValue('custbody_tn_complete');
			if (isSubmit != 'T')
				alert('Your purchase request is NOT submitted. Current information will be saved as a draft.');
		}
