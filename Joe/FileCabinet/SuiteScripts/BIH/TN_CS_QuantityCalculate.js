...
define(
		...
		function(record, search) {
			...
			function fieldChanged(context) {
				...
				if (sublistName === 'item'
						&& (sublistFieldName === 'custcol_xsdd_gmsl' || sublistFieldName === 'custcol1')) {
					var buyQuantity = currentRecord.getCurrentSublistValue({// 购买数量
						sublistId : 'item',
						fieldId : 'custcol_xsdd_gmsl'
					});
					var servicePeriod = currentRecord.getCurrentSublistValue({// 服务期限
						sublistId : 'item',
						fieldId : 'custcol1'
					});
//					servicePeriod = servicePeriod.indexOf('月') >= 0 ? servicePeriod
//							.substring(0, servicePeriod.length - 2)
//							: null;
					if (buyQuantity && servicePeriod) {
						var quantity = buyQuantity * servicePeriod;
						currentRecord.setCurrentSublistValue({// Quantity
							sublistId : 'item',
							fieldId : 'quantity',
							value : quantity
