 ...
 */
define(['N/ui/serverWidget', 'N/record', 'N/redirect', 'N/search', 'N/error', 'N/url','N/currency'],

function(serverWidget, record, redirect, search, error, url,currencyF) {
        ...
        try {
            ...
            if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT) {
                ...
                for (var i = 0; i < creCount.length; i++) {
                    ...
                    if (currencysCre.indexOf(currency) == '-1' && isSelect == 'T') {
                        currencysCre.push(currency);
                    }
                }
                for (var i = 0; i < currencysCre.length; i++) {
                    var creMap = {
                        currency: currencysCre[i],
                        credits: {}
                    };
                    for (var j = 0; j < creCount; i++) {
                        ...
                        if (isSelect == 'T' && payment && currency == currencysCre[i]) {
                            creMap.credits[recordId] = payment;
                        }
                    }
                    ...
                    for (var j = 0; j < creditList.length; j++) {
                        if (creditList[j].currency == invoiceList[i].currency) {
                            ...
                            for (var k = 0; k < creNum; k++) {
                                ...
                                if (creditList[j].credits[internalid]) {
                                    var payment = creditList[j].credits[internalid];
                                    // 动态模式下（DynamicRecord）用下面三个步骤设置line上的值
                                    var lineNum = cpayRecord.selectLine({
                                        sublistId: 'credit',
                                        line: k
    ...
    // });
                ...
                for (var i = 0; i < curAccs.length; i++) {
                    ...
                    for (var j = 0; j < apCount; j++) {
                        ...
                        if (isSelect == 'T' && payment && currencyLine == currency && apAccountLine == apAccount) {
                            billMap.bills[recordId] = payment;
                        }
                    }
                ...
                for (var i = 0; i < billList.length; i++) {
                    ...
                    for (var j = 0; j < apNum; j++) {
                        ...
                        if (billList[i].bills[internalid]) {
                            var payment = billList[i].bills[internalid];
                            // 动态模式下（DynamicRecord）用下面三个步骤设置line上的值
