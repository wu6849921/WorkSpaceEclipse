		...
		function(record, search, currency) {
			...
			function fieldChanged(context) {
				try {
					...
					// 监听itemProject
					if (sublistFieldName == 'custbody_tn_item_filter') {
						var itemF = currentRecord.getValue({
							fieldId : 'custbody_tn_item_filter',
						});
						// 先删除list
						var numLines = currentRecord.getLineCount({
							sublistId : 'recmachcustrecord_tn_projectlist'
						});
						for (var i = 0; i < numLines; i++) {
						...
						for (var i = 0; i < itemLines; i++) {
							var _item = currentRecord.getSublistValue({
								sublistId : 'item',
								fieldId : 'item',
								line : i
							});
							// var _line = currentRecord.getSublistValue({
							// sublistId : 'item',
							// fieldId : 'line',
							// line : i
							// });
							var _displayName = currentRecord.getSublistValue({
								sublistId : 'item',
								fieldId : 'custcol_tn_opp_display_name',
								line : i
							});
							var estimitfcost = currentRecord.getSublistValue({
								sublistId : 'item',
					...
					// 监听items
					if (sublistName === 'recmachcustrecord_tn_projectlist') {
						if (sublistFieldName === 'custrecord_tn_items_mdstifaccost') {
							var estimitfcost = currentRecord
									.getCurrentSublistValue({
										sublistId : sublistName,
										fieldId : sublistFieldName
									});
							var _line = currentRecord.getCurrentSublistValue({
								sublistId : sublistName,
								fieldId : 'custrecord_tn_items_itemline'
							});
							var lineNum = currentRecord.selectLine({
