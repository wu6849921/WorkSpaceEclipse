...
function write(request, response) {
	try {
		...
		// nlapiLogExecution('debug', 'invoiceNumber', info.invoiceNumber);
		info.invoiceCode = request.getParameter('invoiceCode');

		info.invoiceDate = request.getParameter('invoiceDate');
		// nlapiLogExecution('debug',
		// 'invoiceId|invoiceNumber|invoiceCode|invoiceDate',
		// info.invoiceId + '|' + info.invoiceNumber + '|'
		// + info.invoiceCode + '|' + info.invoiceDate);
...
function writeInfo(info) {
	try {
		...
		// var filters = [];
		// nlapiLogExecution('debug', 'invoiceNumber', info.invoiceId);
		// filters[filters.length] = new nlobjSearchFilter('tranid', null, 'is',
		// info.invoiceId);
		var rts = nlapiSearchRecord('transaction', null,
				[ new nlobjSearchFilter('tranid', null, 'is', info.invoiceId) ]);
		// var rts = nlapiSearchRecord('transaction', null, filters);
		// nlapiLogExecution('debug', 'rts', rts);
		if (!rts)
		...
		var val = [ '4', info.invoiceNumber + info.invoiceCode,
				info.invoiceDate ];
		nlapiLogExecution('debug', 'recType|invId|key|val', recType + '|'
				+ invId + '|' + key + '|' + val);
