				...
				if(getTranid.indexOf(customForm) != -1) {
					...
					if(scriptContext.mode != 'delete')
						...
						if(scriptContext.type == scriptContext.UserEventType.EDIT || scriptContext.type == scriptContext.UserEventType.CREATE)
						{
							...
							//-----Search for Reversal code by search--------------------//
							var taxCodeFilters = [];
							var taxCodeColumns = [];
							...
							for(var k=0; k<=expense_count; k++)
							{
								...
								if(rev_apply)
								{
									...
									//revPercent = revPercent.substring(0,revPercent.length - 1);
									...
									//strevpercent = strevpercent.substring(0,strevpercent.length - 1);
								}
							}
							...
							//var gsttype =billObject.getValue({sublistId: 'item'});
							...
							for(var k=0; k <item_count; k++)
							{
								...
								if(selfBilling == true || purType == 2 || unbil == true || unbil == 'T')
								{
									...
									if(revcheck == true || revcheck == 'T')
									{
										...
										if(arrSearchResults[0]){
											...
											//var reversalTaxCode = arrSearchResults[0].getValue('custrecord_gst_reversal_tax_code')
								...
								if(rev_apply)
								{
									...
									//var strevpercent = billObject.getSublistValue({sublistId:'item',fieldId:'custcol_st_rev_percent',line:k});
									...
									if(gstType == intra)
									{
										...
										var purchaseAmountItem = amountItem * (cgstRate/100); // cgst amount
										var negativeAmount = -purchaseAmountItem;

										var purchaseAmountsgst = amountItem * (sgstRate/100); // sgst amount
								...
								else
								{
									...
									//-------------Reversal Apply is not true END---------------//
	 ...
	 function setLineItems(currentRecordObj){
		...
		log.debug('sezInvvvvvvv',sezInvoice);
	    	...
	    	for(var i = 0; i < itemCount; i++){
	    		...
	    		if(itemid >1)
	    		{
	    			//currentRecordObj.setSublistValue({sublistId: 'item',fieldId:'custcol_item_schedule_gst',line:i,value:getSchedule});
