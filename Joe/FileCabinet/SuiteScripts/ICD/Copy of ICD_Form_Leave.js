...
function saverecord() {
	...
	// joe
	var emtype = nlapiLookupField('employee', employee, 'custentity_noemployee');
	if (emtype == 'T') {
	...
	try {
		if (leavetype == 20) {
			...
			// joe
			var AL_lastyear_remaining = parseFloat(nlapiLookupField('employee',
		...
		if (leavetype == 21) {
			...
			// joe
			var CL_lastyear_remaining = parseFloat(nlapiLookupField('employee',
					nlapiGetFieldValue('custrecord324'),
...
function checkAlRemain() {
	...
	if (YearNow == LeaveYear) {
		if (nlapiGetFieldValue('custrecord344') == 1) {
			...
			var leaveType = nlapiGetFieldValue('custrecord325');
			// joe
			var remainingAL = parseFloat(nlapiLookupField('employee', Employee,
					'custentity176'));
...
function judgeTrialDate() {
	...
	var Employee = nlapiGetFieldValue('custrecord324');
	// joe
	ProbationEndDate = nlapiStringToDate(nlapiLookupField('employee', Employee,
			'custentity89'));
	...
	var leaveType = nlapiGetFieldValue('custrecord325');
	// joe
	var remainingAL = nlapiLookupField('employee', Employee, 'custentity176');
