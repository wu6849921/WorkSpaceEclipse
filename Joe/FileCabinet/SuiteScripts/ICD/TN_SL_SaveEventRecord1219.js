		...
		if (!integration.validateLoginInfo(username, password, null)) {
			...
			if (caseID) {
				...
				if (isCase == 'T') {
					...
					var eventRecordID = nlapiSubmitRecord(eventRec);// check the
					// ID
					...
					if (caseStatus) {
						...
						// 记录第一次 状态为14-Case Responded To(Case)更新时间 joe 181205
						if (caseStatus == '14' && (caseStatus != caseStatusOld)
								&& !caserestoDate) {
							caseRcrd.setFieldValue(
									'custevent_tn_caserestodate',
					...
					for (var i = 1; i <= parseInt(caseImageCount); i++) {
						var caseImage = request.getFile('caseImage'
								+ caseImageCount);
						var fileID;
						if (caseImage) {
					...
					var file = request.getParameter('eventAttach');
					...
					caseRcrd.setFieldValue('custevent_2nd_departure_time',
							sDepartureTime);
					caseRcrd.setFieldValue('custevent_rpmimplementationdate',
					...
					if (file) {
						...
						if (templateId) {
							...
							nlapiSendEmail(author, recipient, mergeResult
									.mergeResult(), mergeResult.getBody(),
									null, null, null, fileCreated);
						}
						// ////////////////////////////////////////////////////
						// 保存文件到系统
						fileID = nlapiSubmitFile(fileCreated);
