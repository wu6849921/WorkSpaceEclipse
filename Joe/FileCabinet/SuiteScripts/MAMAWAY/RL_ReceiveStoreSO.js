...
define(['N/record', 'N/format', 'N/search', 'N/runtime', 'N/https', 'N/url'],
    function (record, format, search, runtime, https, url) {
            ...
            try {
                if (POSType == 0) {
                    ...
                    if (succeed) {
                        ...
                        if (dataIn.location) {
                            ...
                            if (isEmpty(locationId)) {
                                var result = {
                                    success: false,
                                    message: "Warehouse can not be located"
                                };
                                var logId = writeLog(dataIn, result);
                                return result;
                            }
                        ...
                        _setText(soRec, 'custbody_logcom', dataIn.custbody_iflogcom);
                    ...
                    for (var i = 0; i < items.length; i++) {
                        if (isCreate) {
                            ...
                            _setCurrentValue(soRec, 'item', 'tax1amt', i, items[i].tax1amt);
                            items[i].isclosed = convertBool(items[i].isclosed);
                    ...
                    // Transform sales order to fulfillment
                    if (true) {
                        try {
                            ...
                            //soObject.message = 'Sales order ' + soId + ' is fulfilled';
                        }
                        catch (fulfillEx) {
                    ...
                    if (dataIn.payments && false) {
                        ...
                        for (var i = 0; i < deposits.length; i++) {
                            ...
                            try {
                                ...
                                _setValue(depositRec, 'checknum', 'CD-' + dataIn.tranid + '-' + i);
                    ...
                    if (dataIn.payments) {
                        ...
                        for (var i = 0; i < paySchedules.length; i++) {
                            ...
                            //paySchedules[i].methodid = getListId(methodName, paramJSON.method);
                ...
                if (POSType == 1) {
                    var restUrl = url.resolveScript({
                        scriptId: 'customscript_mmy_pos_04_01_type_1',
                        deploymentId: 'customdeploy_mmy_pos_04_01_type_1',
                ...
                if (POSType == 2) {
                    var restUrl = url.resolveScript({
                        scriptId: 'customscript_mmy_receivesalesreturn',
                        deploymentId: 'customdeploy_mmy_receivesalesreturn',
                        returnExternalUrl: true
                    });
                    log.debug('restUrl', restUrl);
        ...
        function writeLog(dataIn, response) {
            var scriptObj = runtime.getCurrentScript();
            var scriptId = scriptObj.getParameter({ name: 'custscript_mmy_pos040100' });
