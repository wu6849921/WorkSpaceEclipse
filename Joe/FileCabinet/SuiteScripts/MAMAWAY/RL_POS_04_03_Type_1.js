    ...
    function (record, format, search, runtime) {
        function _do_post(dataIn) {
            ...
            try {
                ...
                //if (succeed && !succeed) {
                if (succeed && dataIn.type == 1 && false) {
                    try {
        ...
        function writeLog(dataIn, response) {
            var scriptObj = runtime.getCurrentScript();
            var scriptId = scriptObj.getParameter({ name: 'custscript_mmy_pos040301' });
        ...
        function validateSalesOrder(soId, context) {
            ...
            if (isEmpty(searchList)) {
                ...
                return { success: true, message: 'transaction cleared' };
            }
