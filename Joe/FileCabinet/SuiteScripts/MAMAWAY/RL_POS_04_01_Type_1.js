...
define(['N/record', 'N/format', 'N/search', 'N/runtime', 'N/log'],
    function (record, format, search, runtime, _log) {
        function _do_post(dataIn) {
            ...
            try {
                ...
                if (succeed) {
                    // _log.debug({ title: 'dataIn', details:
                    ...
                    // _log.debug({ title: 'soId', details: soId });
                    ...
                    if (isEmpty(soId)) {
                        ...
                        // _log.debug({ title: 'validation', details: validation
						// });
                    ...
                    if (isEmpty(soRec)) {
                        var result = {
                            success: false,
                            message: "Sales order can not be located"
                        };
                        var logId = writeLog(dataIn, result);
                        return result;
                    }
                    ...
                    // var customerId = getIdByExternalId(dataIn.entity,
					// 'customer');
                ...
                if (dataIn.payments) {
                    ...
                    for (var i = 0; i < deposits.length; i++) {
                        ...
                        try {
                            ...
                            _setValue(depositRec, 'custbody_mmy_ccnumber', deposits[i].ccnumber);
                            ...
                            _log.debug({ title: i, details: 'deposit: ' + depositId });
                            soObject.depositid = depositId;
                        }
                        catch (depositEx) {
                            _log.debug({ title: i, details: 'deposit error: ' + depositEx });
                ...
                if (succeed && false) {
                    try {
                        ...
                        _log.debug({ title: 'invoiceId', details: invoiceId });
                        soObject.invoiceid = invoiceId;
                    }
                    catch (invoiceEx) {
                        _log.debug({ title: 'invoice error', details: invoiceEx });
                        if (isCreate) record.delete({ type: 'salesorder', id: soId });
                ...
                if (dataIn.payments) {
                    ...
                    for (var i = 0; i < paySchedules.length; i++) {
                        ...
                        // paySchedules[i].methodid = getListId(methodName,
						// paramJSON.method);
                        ...
                        try {
                            ...
                            // _setValue(schedule, 'custrecord_pos_trnx',
							// invoiceId);
                            ...
                            // _log.debug({ title: i + ', schedule id', details:
							// scheduleId });
                        }
                        catch (paymentEx) {
                            _log.debug({ title: 'paymentEx', details: paymentEx });
                            soObject.success = false;
                ...
                var log = writeLog(dataIn, soObject);
                return soObject;
            }
            catch (ex) {
                _log.debug({ title: '_post', details: ex });
                soObject.success = false;
                soObject.message = ex;
                var log = writeLog(dataIn, soObject);
                return soObject;
        ...
        function writeLog(dataIn, response) {
            var scriptObj = runtime.getCurrentScript();
            var scriptId = scriptObj.getParameter({ name: 'custscript_mmy_pos040101' });
            ...
            logRec.setValue({ fieldId: 'custrecord_intlog_apiid', value: 3 });
        ...
        function validateSalesOrder(soId, context) {
            ...
            var filters = new Array();
            filters.push([['createdfrom', 'anyof', [soId]], 'or', ['applyingtransaction', 'anyof', [soId]]]);
            filters.push('and');
            ...
            for (var i = 0; i < searchList.length; i++) {
                ...
                // log.debug({ title: tranId, details: context.closedate + ', '
        ...
        function getIdByExternalId(externalId, type) {
            ...
            // _log.debug({ title: 'search', details: externalId + ', ' +
			// JSON.stringify(existList) });
        ...
        function getSubRecId(soId, type, search) {
