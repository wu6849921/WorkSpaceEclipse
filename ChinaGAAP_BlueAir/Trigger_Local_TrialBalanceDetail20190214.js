/**
 * Module Description: Trial Balance Detailed Version Date Author Remarks 1.00 19 May 2017 Veronica
 */
...
trigger.local.trialbalancedetail.prototype = {
	writeTrialBalanceDetailToPDF : function(subsidiaryid, periodid, period, monthFromID, monthFromText, monthToID, monthToText, response) {
		var filter = [];
		var filters = [];
		...
		if (monthFromID && monthFromText && monthToID && monthToText) {
			...
			var accPeriodStartDate = accPeriodFrom.getFieldValue('startdate'); // format of date M/D/YYYY, no leading zeros
			...
			var periodtimeStart = String.format("{0}/{1}/{2}", startMonth, startDay, startYear);
			var beginningOfYearPeriod = String.format("{0}/{1}/{2}", 1, 1, startYear);

			// create filters for posting period ids [monthFromID, monthFromID+1,.. monthToID]
			var periodFromToFilters = this.prepareFilterPostingPeriodFromToPeriodIDs(monthFromID, monthToID);

			// add the created filters to the common list
			for (var t = 0; t < periodFromToFilters.length; t++) {
				filter[filter.length] = periodFromToFilters[t]; // this month amount
			}

			filters[filters.length] = new nlobjSearchFilter('startdate', 'accountingperiod', 'before', periodtimeStart); // opening balance filter

			incomeAccountsFilters[incomeAccountsFilters.length] = new nlobjSearchFilter('startdate', 'accountingperiod', 'onorafter', beginningOfYearPeriod);
			incomeAccountsFilters[incomeAccountsFilters.length] = new nlobjSearchFilter('startdate', 'accountingperiod', 'before', periodtimeStart);
			...
			filters[filters.length] = new nlobjSearchFilter('startdate', 'accountingperiod', 'before', periodtime);
		...
		for (var m = 0; m < 2; m++) {
			...
			if (m == 0) {
				incomeAccountsTempmap[m] = rule.GetSummaryLedgerRecordsForIncomeAccounts(incomeAccountsFilters, column); // starting from new year opening Balance
			}

			tempmap[m] = rule.GetSummaryLedgerRecordsByAllOfNumber(filters, column); // Opening Balance, 期初数据
			tempmonthmap[m] = rule.GetSummaryLedgerRecordsByAllOfNumber(filter, column); // Occuring amount this period, 本月数据
		}
		...
		var incomeAccountsBeginBalance = []; // at the beginning of the period for income (P&L) accounts
		...
		if (fromToPeriodLbl) {
			xml += template.GetTrialBalanceContentHeadXML(fromToPeriodLbl, currPageNumber++);
		} else {
			xml += template.GetTrialBalanceContentHeadXML(period, currPageNumber++);
		}
		...
		for (var i = 0; i < resultslice.length; i++) {
			...
			cnname = com.formatStringValuesInXml(resultslice[i].getValue('custrecord_trigger_name_coa_cn', null, null));
			var coaname = cnnumber + ' ' + cnname;
			var sum = Object.create(trigger.local.entity.TrialBalance);
			sum.name = coaname;

			// handle special case for P&L accounts of period 1/xxxx, then starting credit and debit should be 0 , then accumulating for the chosen period from the beginning of the year
			...
			if (incomeAccountsMap.Contains(cnnumber)) {// if it's P&L account collect data from befinning of this year untill now (not including current period)
			// nlapiLogExecution('DEBUG', 'checking if P&L account', cnnumber + ' account is P&L account <-- IT IS P&L account, incomeAccountsBeginBalance=' + incomeAccountsBeginBalance);
			...
			begbalance = parseFloat(sum.beginDebit) - parseFloat(sum.beginCredit);
			if (begbalance > 0) {
			...
			endbalance = begbalance + parseFloat(sum.currentDebit) - parseFloat(sum.currentCredit);
	...
	/**
	 * Prepare filter for quering transaction based on posting periods filter in-between the provided period ids
	 * 
	 * @returns filter on posting periods including the range between provided values
	 */
	prepareFilterPostingPeriodFromToPeriodIDs : function(periodFromId, periodToId) {
		var periodIds = [];
	...
	/**
	 * Return the list of accounting periods without Quater definition as map with accounting period ids
	 * 
	 * @returns
	 */
