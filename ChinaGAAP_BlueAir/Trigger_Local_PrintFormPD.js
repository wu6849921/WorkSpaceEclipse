...
function clientPageInit() {
	...
	// Make the fields MonthFrom & MonthTo hidden. They will be available for
	// TrialBalance Report
...
trigger.local.printform.prototype = {
	...
	main : function(request, response) {
		...
		form
				.addButton('custpage_trialblnc_dtl_button',
						ren.DetailedTrialBalance,
						"new trigger.local.printform().redirectToDetailedTrialBalancePrinting()");
		...
		//
		// }
		// 鏀规垚鏍规嵁鐧婚檰浜烘墍鍦ㄥ叕鍙歌缃叕鍙稿垪琛紝濡傛灉鐧婚檰瑙掕壊涓虹鐞嗗憳鍒欏彲浠ョ湅鍒颁袱瀹跺叕鍙� edit by joe 20190114
		var roleId = nlapiGetRole();
		// nlapiLogExecution('debug', 'roleName', roleName);
		if (roleId == '3' || roleId == '18') {// 濡傛灉鐧婚檰瑙掕壊绛変簬administer鎴栬�協ull
			// access鍒欑湅鍒颁袱瀹跺叕鍙�
			my_subsidiary.addSelectOption('8',
					'Blueair (Shanghai) Sales Co., Ltd.');
			my_subsidiary.addSelectOption('13', 'Blueair Technology');
		} else {
			var subsidiaryId = nlapiGetSubsidiary();
			var subsidiaryName;
			if (subsidiaryId == '8') {
				subsidiaryName = 'Blueair (Shanghai) Sales Co., Ltd.';
			} else if (subsidiaryId == '13') {
				subsidiaryName = 'Blueair Technology';
			}
			if (subsidiaryName) {
				my_subsidiary.addSelectOption(subsidiaryId, subsidiaryName);
			}
		}
		...
		var monthPeriodFromFld = form.addField('custpage_monthperiodfrom',
				'select', ren.MonthFrom, null, 'parametersgroup');
		...
		var monthPeriodToFld = form.addField('custpage_monthperiodto',
				'select', ren.MonthTo, null, 'parametersgroup');
		...
		// 绉戠洰
		// 鏂板绛涢�夋潯浠� subsidiary edit by joe 20190114
		var _subsidiaryId;
		if (roleId != '3' && roleId != '18') {// 濡傛灉涓嶆槸绠＄悊鍛樺垯澧炲姞鍏徃绛涢�夋潯浠�
			_subsidiaryId = nlapiGetSubsidiary();
		}
		var htmlall = this.HTMLForAllAccountLists(_subsidiaryId);
//		nlapiLogExecution('debug', 'htmlall', htmlall);
	...
	OnSelectYearPeriodChange : function() {
		...
		for (var i = 0; i < n1.options.length; i++) {
			...
			if (str == year.value) {
				nlapiInsertSelectOption('custpage_monthperiodfrom', o.value,
						o.text, false);
				nlapiInsertSelectOption('custpage_monthperiodto', o.value,
						o.text, false);
				n2.options.add(o);
			}
		}
	...
	HTMLForAllAccountLists : function(_subsidiaryId) {

		var html = '<div style="display:none;"><select name="allaccountlist" id="allaccountlist" multiple="true">';

		html += this.CreateChartOfAccount("1,2,3", _subsidiaryId);
	...
	CreateChartOfAccount : function(level, _subsidiaryId) {

		// 绉戠洰

		var map = this.GetCNCOAListByLevel(level, _subsidiaryId);
	...
	GetCNCOAListByLevel : function(internalids, _subsidiaryId) {
	...
	OnSlectItemChange : function() {
		...
		} else if (o.value == 5 || o.value == 7 || o.value == 2) {// for Trial
			// Balance
			// report
			// (5) or
			// Detailed
	...
	returnParameters : function() {
		...
		} else if ((type.value == 5 || type.value == 7) && monthFromPeriodID
				&& monthToPeriodID) {// Trial Balance report: don't use
			// period and period id when MonthFrom &
			// MonthTo are chosen
