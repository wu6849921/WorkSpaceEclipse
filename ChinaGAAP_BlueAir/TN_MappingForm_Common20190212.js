...
trigger.local.mappingcommon.prototype = {
	...
	createFormBody : function(form) {
		try {
			...
			var one2ManyChkBox = form.addField(
					'custrecord_trigger_one_to_many', 'checkbox',
			...
			this.populateListWithSavedSearchResults('location',
					'customsearch_trigger_locations_clsfctn', locationSelect);
		...
		for (var i = 0; i < 200; i++) {
			...
			if (resultslice.length <= 0) {
				...
				for (var j = 0; j < resultslice.length; j++) {
					internalid = resultslice[j].getValue('internalid', null,
							'GROUP');
					name = resultslice[j].getValue('name', null, 'GROUP');
					map[internalid] = name;
				}
			}
	...
	populateMappingFormWithExistingRecordData : function(origRecordID) {
		try {
			...
			if (isO2MMapping == 'T') {
				...
				nlapiSetFieldValue('custrecord_trigger_ns_location', lctn,
						false, true);
