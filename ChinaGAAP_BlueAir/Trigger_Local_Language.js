...
trigger.local.language.prototype = {
	...
	UIoptions : function(type) {
		...
		default:
			temp = {
				...
				DetailedTrialBalance : 'Detailed Trial Balance',
	...
	UIType_ZH_CN : function() {
		...
		html += '<a class="smallgraytextnolink">报表类型</a></span></div>';//Report type
		...
		html += '<option value=7 >-- 试算平衡表（全）</option>';//Detailed Trial Balance
		html += '</select></div>';
		return html;
	},
	...
	UIType_EN_US : function() {
		...
		html += '<option value=7 >-- Detailed Trial Balance</option>';
		html += '</select></div>';
		return html;
	},
