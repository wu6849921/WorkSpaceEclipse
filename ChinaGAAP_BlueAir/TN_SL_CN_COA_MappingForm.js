...
function suitelet(request, response) {
	try {
		...
		// nlapiLogExecution('DEBUG', 'suitelet', 'TN_SL_CN_COA_MappingForm,
		// type is: ' + type + ' Orig record ID is: ' + origRecordID);
		...
		if (recordMode == 'create') {
			...
			form.addButton('custpage_btn_add', displayLbl.add,
					'addMappingToTheWaitingList()');
		} else if (recordMode == 'edit') {
			form.addButton('custpage_btn_edit', displayLbl.editLbl,
					'editMappingRecord(' + origRecordID + ')');
			form.addButton('custpage_btn_dlt', displayLbl.deleteLbl,
					'deleteMappingRecord(' + origRecordID + ')');
		} else if (recordMode == 'view') {
			mappingCommon
					.populateMappingFormWithExistingRecordData(origRecordID);

			// mark all the fields as disabled
			form.getField('custpage_chinaaccountname').setDisplayType(
					'disabled');
			form.getField('custrecord_trigger_ns_subsidiary').setDisplayType(
					'disabled');
			form.getField('isinactive').setDisplayType('disabled');
			form.getField('custrecord_trigger_one_to_many').setDisplayType(
					'disabled');
			form.getField('custrecord_trigger_ns_department').setDisplayType(
					'disabled');
			form.getField('custrecord_trigger_ns_location').setDisplayType(
					'disabled');
			form.getField('custrecord_trigger_ns_class').setDisplayType(
					'disabled');
		}
...
function initMappingSublist(form) {
	...
	var sublist = form.addSubList('custpage_sublist_cncoa_mapping',
			'inlineeditor', displayLbl.mappingLbl);// adds a sublist area to
	...
	sublist.addField('custpage_is_mapping_selected', 'checkbox',
			displayLbl.selected);
	sublist.addField('custpage_china_acc_name', 'text',
			displayLbl.chinaAcntNameLbl).setDisplayType('disabled');
	...
	sublist.addField('custpage_sbsdry_name', 'text', displayLbl.sbsdryNameLbl)
			.setDisplayType('disabled');
	sublist.addField('custpage_mapping2many', 'checkbox',
			displayLbl.one2manyChckBoxLbl).setDisplayType('disabled');
	sublist.addField('custpage_deptmnt_name', 'text',
			displayLbl.departmentNameLbl).setDisplayType('disabled');
