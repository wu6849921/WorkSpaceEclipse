/**
 * Module Description: Profit & Loss Version Date Author Remarks 1.00 23 Oct 2014 Winson.Chen
 */
...
trigger.local.subledger.prototype = {
	writeSubLedgerToPDF : function(subsidiaryid, periodid, period, accountid, response, monthFromID, monthFromText, monthToID, monthToText) {
		...
		if (internalids.length > 0) {
			prevfilter[0] = new nlobjSearchFilter('internalid', 'account', 'anyof', internalids);
		...
		if (subsidiaryid && subsidiaryid != '-1') {
			prevfilter[prevfilter.length] = new nlobjSearchFilter('subsidiary', null, 'anyof', subsidiaryid);
		...
		if (monthFromID && monthFromText && monthToID && monthToText) {
			...
			var sd = nlapiLookupField('accountingperiod', monthFromID, 'startdate');
			var ed = nlapiLookupField('accountingperiod', monthToID, 'enddate');

			prevfilter.push(new nlobjSearchFilter('startdate', 'accountingperiod', 'before', sd));
			filter.push(new nlobjSearchFilter('startdate', 'accountingperiod', 'onorafter', sd));
			filter.push(new nlobjSearchFilter('startdate', 'accountingperiod', 'onorbefore', ed));
			filters.push(new nlobjSearchFilter('startdate', 'accountingperiod', 'onorafter', sd));
			filters.push(new nlobjSearchFilter('startdate', 'accountingperiod', 'onorbefore', ed));
			// filters.push(new nlobjSearchFilter('startdate', 'accountingperiod', 'within', sd, ed));
			...
			var periodtime = String.format("{0}/{1}/{2}", endMonth, endDay, endYear);// end of chosen period

			var begintime = '1/1/' + year;

			// 期初余额时间
			var prevtime = String.format("{0}/{1}/{2}", startMonth, startDay, startYear);// beginning of chosen period

			prevfilter[prevfilter.length] = new nlobjSearchFilter('startdate', 'accountingperiod', 'before', prevtime);
			filter[filter.length] = new nlobjSearchFilter('postingperiod', null, 'abs', [ periodid ]);
			filters[filters.length] = new nlobjSearchFilter('startdate', 'accountingperiod', 'within', begintime, periodtime);
		}
		...
		// content for pdf
		var xml = this.GetSubLedgerLists(filter, period, columns, entity.map, initmap, totalmap);
		if (xml) {
	...
	GetTotalAmounts : function(filters, columns) {
		...
		for (var i = 0; i < resultslice.length; i++) {
			...
			map.Add(accnumber, new trigger.local.HashEntity(accnumber, [ credit, debit ]));
		}
		return map;
	},
	...
	GetSubLedgerLists : function(filters, year, columns, map, initmap, totalmap) {
		...
		for (var i = 0; i < resultslice.length; i++) {
			...
			year = y.toString() + '-' + monthStr;

			var nn = 0;// nn var holds max amount of lines between number, memo & account name values
			...
			if (isfirst == true) {
				...
				pages.push(page);
				xml += template.GetSubLedgerContentHeadXML(year, accnumber + ' ' + account, page, "{0}");
			...
			if (isfirst == false || accnumber == next_accnumber) {
				...
				xml += template.GetSubLedgerAccountXML(month, day, number, nlapiEscapeXML(memo), account, com.formatCurrency(debit), com.formatCurrency(credit), com.formatCurrency(balance));
				x++;
			}
			// total

			if (isfirst == false && ((i == resultslice.length - 1 || x % m == m - 1) || accnumber != next_accnumber)) {
				if (x % m == m - 1) {
					page++;
				} else {
					page = 1;
				}
				if (accnumber != next_accnumber || (i == resultslice.length - 1)) {
				...
				if (x % m != m - 1) {
					...
					xml += template.GetTotalBalanceXML(month, '本月累计', com.formatCurrency(monthdebit), com.formatCurrency(monthcredit), com.formatCurrency(balance));
					...
					xml += template.GetTotalBalanceXML(month, '本年累计', com.formatCurrency(yeardebit), com.formatCurrency(yearcredit), com.formatCurrency(balance));
