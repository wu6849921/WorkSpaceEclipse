...
trigger.local.printform.prototype = {
	...
	main : function(request, response) {
		...
		//
		// }
		// 改成根据登陆人所在公司设置公司列表，如果登陆角色为管理员则可以看到两家公司 edit by joe 20190114
		var roleId = nlapiGetRole();
		// nlapiLogExecution('debug', 'roleName', roleName);
		if (roleId == '3' || roleId == '18') {// 如果登陆角色等于administer或者full
			// access则看到两家公司
			my_subsidiary.addSelectOption('15',
					'Blueair (Shanghai) Sales Co., Ltd.');
			my_subsidiary.addSelectOption('17', 'Blueair Technology');
		} else {
			var subsidiaryId = nlapiGetSubsidiary();
			var subsidiaryName;
			if (subsidiaryId == '15') {
				subsidiaryName = 'Blueair (Shanghai) Sales Co., Ltd.';
			} else if (subsidiaryId == '17') {
		...
		// 科目
		// 新增筛选条件 subsidiary edit by joe 20190114
		var _subsidiaryId;
		if (roleId != '3' && roleId != '18') {// 如果不是管理员则增加公司筛选条件
