...
trigger.local.trialbalance.prototype = {
	...
	writeTrialBalanceToPDF : function(subsidiaryid, periodid, period, monthFromID, monthFromText, monthToID, monthToText, response) {
		var filter = [];
		var filters = [];
...
//			nlapiLogExecution('debug', 'TRIAL-SQL', sql);
		...
		if(fromToPeriodLbl){
			xml += template.GetTrialBalanceContentHeadXML(fromToPeriodLbl, 1);
		}else{
...
//				nlapiLogExecution('DEBUG', 'checking  if P&L account', cnnumber + ' account is P&L account <-- IT IS P&L account, incomeAccountsBeginBalance=' + incomeAccountsBeginBalance);
			...
			if(m == 45){
				...
				if(fromToPeriodLbl){
					xml += template.GetTrialBalanceContentHeadXML(fromToPeriodLbl, 2);
				}else{
...
//				nlapiLogExecution('DEBUG', 'prepare Map Of P&L account numbers', 'account number = ' + cnnAcntNumber);
	...
	prepareFilterPostingPeriodFromToPeriodIDs : function(periodFromId, periodToId){
		var periodIds = [];
		nlapiLogExecution('DEBUG', 'prepareFilterPostingPeriodFromToPeriodIDs', 'periodFromId: ' + periodFromId + ' periodToId: ' + periodToId);
