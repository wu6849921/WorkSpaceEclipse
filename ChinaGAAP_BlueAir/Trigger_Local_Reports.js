...
function main(request, response) {
	...
	var monthFromID = request.getParameter('MONTHFROMID');
	var monthFromText = request.getParameter('MONTHFROM');
	
	var monthToID = request.getParameter('MONTHTOID');
	var monthToText = request.getParameter('MONTHTO');
	...
	switch ( type )
	{
		...
		case 'SUBLEDGER':
			...
			o.writeSubLedgerToPDF(subsidiaryid, periodid, period, accountid, response, monthFromID, monthFromText, monthToID, monthToText);
			break;
		case 'DIARYLEDGER':
		...
		case 'TRIALBALANCE':
			...
			o.writeTrialBalanceToPDF(subsidiaryid, periodid, period, monthFromID, monthFromText, monthToID, monthToText, response);
			break;
		case 'TRIALBALANCEDETAIL':
			var o = new trigger.local.trialbalancedetail();
			o.writeTrialBalanceDetailToPDF(subsidiaryid, periodid, period, monthFromID, monthFromText, monthToID, monthToText, response);
			break;
		case 'BALANCESHEET':
