...
trigger.local.printform.prototype = {
	...
	main : function(request, response) {
		...
		// 汇总式总账(本期发生)
		form
				.addButton('custpage_summaryledger1_button',
						ren.SummaryLedger1,
						"new trigger.local.printform().redirectToSummaryLedgerPrinting1()");
		form
				.addButton('custpage_summaryledger2_button',
						ren.SummaryLedger2,
						"new trigger.local.printform().redirectToSummaryLedgerPrinting2()");
	...
	redirectToSummaryLedgerPrinting1 : function() {
		...
		suiteletUrl += String.format("&TYPE={0}", 'TRIALBALANCE1');
	...
	redirectToSummaryLedgerPrinting2 : function() {
		...
		suiteletUrl += String.format("&TYPE={0}", 'TRIALBALANCE2');
	...
	DisabledButtonsByID : function(id) {
		...
		arr[7] = 'custpage_summaryledger1_button';
		arr[8] = 'custpage_summaryledger2_button';
	...
	returnParameters : function() {
		...
		if ((type.value == 0) && voucherDateFrom && voucherDateTo) {
			...
			// jdp新增功能：日期只能选择1号-15号或16号-30号
			var dateFroms = voucherDateFrom.split('\/');
			var dateTos = voucherDateTo.split('\/');
			// 得到月份最大天数
			var maxDate;
			if (dateTos[0] == '2') {
				maxDate = parseInt(dateTos[2]) % 4 == 0 ? 29 : 28;
			} else if (dateTos[0] == '1' || dateTos[0] == '3'
					|| dateTos[0] == '5' || dateTos[0] == '7'
					|| dateTos[0] == '8' || dateTos[0] == '10'
					|| dateTos[0] == '12') {
			...
			// alert(parseInt(dateTos[0]) + 1);
			if ((dateFroms[0] == dateTos[0])
					&& ((dateFroms[1] == '1' && dateTos[1] == '15') || (dateFroms[1] == '16' && dateTos[1] == maxDate))) {
				period = null;
				periodid = null;
			} else {
				alert(classlang.GetMsgByVoucherDate(lang));
				return;
			}
