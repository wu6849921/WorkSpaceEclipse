...
trigger.local.rule.prototype = {
	 ...
	 * Joe
	 */
	GetLedgerRecordsByAllOfNumber : function(type, filters) {
		var savedsearch = nlapiLoadSearch('transaction',
		...
		for ( var rs in resultslice) {
			...
			var USDebit = resultslice[rs].getValue(cols[1]);
			var USCredit = resultslice[rs].getValue(cols[2]);
			var adjDebit = resultslice[rs].getValue(cols[3]);
			var adjCredit = resultslice[rs].getValue(cols[4]);
			var localDebit = resultslice[rs].getValue(cols[5]);
			var localCredit = resultslice[rs].getValue(cols[6]);
			var NSAccountNum = resultslice[rs].getValue(cols[7]);
			...
			if (type == 2 || parseInt(NSAccountNum.charAt(0)) < 4) {
				map.Add(NSAccount, new trigger.local.HashEntity(NSAccount, [
						localDebit, localCredit, adjDebit, adjCredit, USDebit,
						USCredit ]));
				// if (NSAccount == '1052') {
				// nlapiLogExecution('debug', 'GetLedgerRecordsByAllOfNumber',
				// NSAccount);
				// }
			}
		}
	 ...
	 * Joe
	 */
	GetLedgerRecordsForIncomeAccounts : function(incomeAccountsFilters) {
		var savedsearch = nlapiLoadSearch('transaction',
				'customsearch_trigger_pl_trial_blnc_2');
		if (incomeAccountsFilters.length > 0) {
			savedsearch.addFilters(incomeAccountsFilters);
			// filters = savedsearch.getFilters();
		}
		var resultset = savedsearch.runSearch();
		var map = new trigger.local.HashTable();
		...
		for ( var rs in resultslice) {
			...
			var cols = resultslice[rs].getAllColumns();
			// var accountName = resultslice[rs].getValue('name', 'account',
			// 'GROUP');
			var NSAccount = resultslice[rs].getValue(cols[0]);
			var USDebit = resultslice[rs].getValue(cols[1]);
			var USCredit = resultslice[rs].getValue(cols[2]);
			...
			if (parseInt(NSAccountNum.charAt(0)) >= 4) {
				// nlapiLogExecution('debug',
				// 'GetLedgerRecordsForIncomeAccounts',
				// parseInt(NSAccountNum.charAt(0)));
				map.Add(NSAccount, new trigger.local.HashEntity(NSAccount, [
						localDebit, localCredit, adjDebit, adjCredit, USDebit,
						USCredit ]));
				// if (NSAccount == '1052') {
				// nlapiLogExecution('debug',
				// 'GetLedgerRecordsForIncomeAccounts', NSAccount);
				// }
			}
	...
	// joe 查询nsaccountid 对应的cnaccountnum+name和nsaccountnum+name
	GetMappingCNCOAParam : function() {
		var savedsearch = nlapiLoadSearch(
				'customrecord_trigger_mapping_cn_coa',
				'customsearch_trigger_mapping_cn_coa');
		var resultset = savedsearch.runSearch();
		...
		for ( var rs in resultslice) {
			var cols = resultslice[rs].getAllColumns();
			var CNAccountNum = resultslice[rs].getValue(cols[0]);
			var CNAccountName = resultslice[rs].getValue(cols[1]);
			var NSAccountId = resultslice[rs].getValue(cols[2]);
			var NSAccountNumName = resultslice[rs].getValue(cols[3]);
			map.Add(NSAccountId, [ NSAccountNumName,
					CNAccountNum + ' ' + CNAccountName ]);
			// nlapiLogExecution('debug', 'NSAccountId', NSAccountNumName + '|'
			// + CNAccountNum + "|" + CNAccountName);

		}
		return map;
	},
	getAccountIdLists : function() {
		var accountList = [];
		var savedsearch = nlapiLoadSearch('transaction',
				'customsearch_trigger_pl_trial_blnc_2');
		...
		for ( var rs in resultslice) {
			var cols = resultslice[rs].getAllColumns();
			var NSAccount = resultslice[rs].getValue(cols[0]);
			var NSAccountName = resultslice[rs].getValue(cols[8]);
			accountList[rs] = NSAccount + '|' + NSAccountName;
