...
trigger.local.generalledger.prototype = {
	writeGeneralLedgerToPDF : function(subsidiaryid, periodid, period,
		...
		if (formula) {
			...
			// WHEN 3 THEN 100101 WHEN 4 THEN 100103 ELSE {account.internalid}
			// END');
			temp.setSort(true);
			cols.push(temp);
		}
		if (internalids.length > 0) {
			...
			var beginbalanceMap = this.GetBalanceByYearBegin(filter, cols); // 骞村垵浣欓,balance
			// at
			// the
			// beginning
			// of
	...
	GetGeneralLedgerLists : function(filters, year, columns, entityMap,
		...
		for (var i = 0; i < resultslice.length; i++) {
			...
			periodEndDate = resultslice[i].getValue('enddate',
		...
		for (var j = 0; j < map.Count; j++) {
			...
			// nlapiLogExecution('debug', 'entityMap',
			...
			xml += template.GetOpeningBalanceByYearXML(t[0], t[1]); // 骞村垵浣欓
	...
	GetBalanceByYearBegin : function(filters, columns) {
		var result = 0;
		var map = new trigger.local.HashTable();
		if (filters.length <= 0) {
			return '';
