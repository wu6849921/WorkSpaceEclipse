...
trigger.local.language.prototype = {
	...
	UIoptions : function(type) {
		...
		case 'zh_CN':
			temp = {
				...
				BalanceSheet : '资产负债表',
				// joe
				BalanceSheet1 : '资产负债表（本期发生）',
				BalanceSheet2 : '资产负债表（期末余额）',
				Subsidiary : '公司名称',
				DateFrom : '起始日期',
				DateTo : '截止日期',
	...
	UIType_ZH_CN : function() {
		...
		// joe
		html += '<option value=7 >-- 试算平衡表（期中）</option>';// Trial Balance
		html += '<option value=8 >-- 试算平衡表（期末）</option>';// Trial Balance
		html += '</select></div>';
		return html;
	},
	...
	GetMsgByVoucherDate : function(lang) {
		...
		if (lang == 'zh_CN') {
			title = '请选择日期为1到15号或者16号到当月的最后一天！ ';
		}
		return title;
	}
