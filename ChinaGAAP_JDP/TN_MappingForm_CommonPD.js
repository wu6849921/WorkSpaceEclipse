 ...
 * Module Description
 * 
 * Version Date Author Remarks 1.00 23 Nov 2016 Veronica
 * 
 */
...
trigger.local.mappingcommon.prototype = {
	...
	HTMLForAccountList : function() {
		...
		html += '<a class="smallgraytextnolink">' + displayLbl.acntNameLbl
				+ '</a></span></div>';
	...
	createFormBody : function(form) {
		try {
			...
			var isInactiveChkBox = form.addField('isinactive', 'checkbox',
					displayLbl.inactiveLbl);

			var html = this.HTMLForAccountList();
			var acc = form.addField('custpage_accountname', 'inlinehtml', null,
					null, null); // ACCOUNT NAME
			acc.setLayoutType('normal', 'none');
			acc.setDefaultValue(html);
			acc.setMandatory(true);

			var chinaAcc = form.addField('custpage_chinaaccountname', 'select',
					displayLbl.chinaAcntLbl);// CHINA ACCOUNT
			var chinaAcntNumberAndNameMap = {};
			var chinaAcntNumberMap = {};
			this.getChinaAccountNameLists(chinaAcntNumberAndNameMap,
					chinaAcntNumberMap);
			...
			var subsidiarySelect = form.addField(
					'custrecord_trigger_ns_subsidiary', 'select',
					displayLbl.sbsdryLbl);
			this
					.populateListWithSavedSearchResults('subsidiary',
							'customsearch_trigger_subsidiars_clsfctn',
							subsidiarySelect);
			subsidiarySelect.setMandatory(true);

			// var one2ManyChkBox =
			// form.addField('custrecord_trigger_one_to_many', 'checkbox',
			// displayLbl.one2manyChckBoxLbl);

			var netsuiteAccnt = form.addField(
					'custrecord_trigger_netsuite_account', 'text',
					displayLbl.nsAccountLbl).setDisplayType('disabled');
			netsuiteAccnt.setLayoutType('normal', 'startcol');

			var accountIDSelect = form.addField(
					'custrecord_trigger_ns_account_id', 'text',
					displayLbl.accountIdLbl).setDisplayType('disabled');
			var chinaAcntNumberSelect = form.addField(
					'custrecord_trigger_cn_account_number', 'select',
					displayLbl.chinaAcntNumberLbl).setDisplayType('disabled');
			...
			var departmentSelect = form.addField(
					'custrecord_trigger_ns_department', 'select',
					displayLbl.departmentLbl);
			departmentSelect.setLayoutType('normal', 'startcol');
			// use saved search to populate department values
			this.populateListWithSavedSearchResults('department',
...
//					'custrecord_trigger_ns_location', 'select',
//					displayLbl.locationLbl);
//			this.populateListWithSavedSearchResults('location',
//					'customsearch_trigger_locations_clsfctn', locationSelect); joe

			var classSelect = form.addField('custrecord_trigger_ns_class',
					'select', displayLbl.classLbl);
			this.populateListWithSavedSearchResults('classification',
					'customsearch_trigger_classes_clsfctn', classSelect);
			...
			common.sendErrorEmail('dev_support@triggerasia.com',
					'FinLocalization', 'TN_SL_CN_COA_MappingForm',
					'createFormBody', ex.message);
			return common
					.getReturnError('TN_SL_CN_COA_MappingForm, createFormBody Error: '
							+ ex.message);
		}
	},
	...
	getAccountNameLists : function() {
		...
		resultset.forEachResult(function(searchResult) {
			internalid = searchResult.getValue('internalid', null, 'GROUP');
			name = searchResult.getValue('name', null, 'GROUP');
			map[internalid] = name;
			return true;
		});
	...
	getChinaAccountNameLists : function(acntNumberAndNameMap, acntNumberMap) {
		...
		var savedsearch = nlapiCreateSearch('customrecord_trigger_cn_coa',
				null, cols);
		var resultset = savedsearch.runSearch();
		for (var i = 0; i < 200; i++) {
			...
			if (resultslice.length <= 0) {
				...
				for (var j = 0; j < resultslice.length; j++) {
					...
					acntNumberMap[internalid] = resultslice[j]
							.getValue(cols[0]);
	...
	populateListWithSavedSearchResults : function(searchRecordType,
			savedSearchID, selectList) {
		var map = {};
		var internalid;
		var name;
		...
		for (var i = 0; i < resultslice.length; i++) {
			...
			// nlapiLogExecution('DEBUG','i = ' + i + 'internalid = ' +
			// internalid +
			// ' name = ' + name);
			map[internalid] = name;
		}
	...
	/**
	 * uploads existing CN COA Mapping record by origRecordID number and
	 * populates the form with the parameters of this record.
	 * 
	 * @param origRecordID
	 * @returns
	 */
	populateMappingFormWithExistingRecordData : function(origRecordID) {
		try {
			...
			// ACCOUNT
			nlapiSetFieldValue('custrecord_trigger_netsuite_account', nsAcc,
					false, true);
			...
			// ACCOUNT
			// ID
			nlapiSetFieldValue('custrecord_trigger_ns_account_id', nsAccID,
					false, true);
			...
			nlapiSetFieldValue('custrecord_trigger_ns_subsidiary', sbsdry,
					false, true);
			...
			// nlapiSetFieldValue('custrecord_trigger_one_to_many',
			// isO2MMapping,
			// true, true);
			var isO2MMapping = 'F';
...
//				nlapiSetFieldValue('custrecord_trigger_ns_location', lctn, joe
//						false, true);

				var cls = recordToEdit
						.getFieldValue('custrecord_trigger_ns_class');
				nlapiSetFieldValue('custrecord_trigger_ns_class', cls, false,
						true);

				nlapiSetFieldDisplay('custrecord_trigger_ns_department', true);
//				nlapiSetFieldDisplay('custrecord_trigger_ns_location', true); joe
...
//				nlapiSetFieldDisplay('custrecord_trigger_ns_location', false);
			...
			common.sendErrorEmail('dev_support@triggerasia.com',
					'FinLocalization', 'TN_MappingForm_Common',
					'populateMappingFormWithExistingRecordData', ex.message);
			return common
					.getReturnError('TN_MappingForm_Common, populateMappingFormWithExistingRecordData Error: '
							+ ex.message);
		}
	},
	...
	getURLForMappingList : function(mappingRcrdID) {
		try {
			var record = nlapiLoadRecord('customrecord_trigger_mapping_cn_coa',
					mappingRcrdID);
			var recordTypeInternalID = record.getFieldValue('rectype');

			var preURL = window.location.href.substring(0, window.location.href
					.indexOf('.com/') + 5);
			var mappingListURL = preURL
					+ 'app/common/custom/custrecordentrylist.nl?rectype='
					+ recordTypeInternalID;
			...
			common.sendErrorEmail('dev_support@triggerasia.com',
					'FinLocalization', 'TN_MappingForm_Common',
					'getURLForMappingList', ex.message);
			return common
					.getReturnError('TN_MappingForm_Common, getURLForMappingList Error: '
							+ ex.message);
		}
	}
